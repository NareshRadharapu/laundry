   ...
   function __construct(){
    ...
    public function customersBalanceList_post(){
		...
		if(is_object($data)){
			...
			try{
				$this->load->database();
			  	$mq = 'select SUM(p.balanceAmount) as balanceAmount, c.cust_id as id, c.firstname as firstname, c.lastname as lastname, c.mobile as mobile from customers as c inner join place_order_ids as p on p.customer_id = c.cust_id where p.isDelete !=1 and c.area_id ='.$storeId.' group by c.cust_id order by c.firstname ';
					$query = $this->db->query($mq);
		...
		if(is_object($data)){
			try{
				if(property_exists($data, "cityId") && $cityId = $data->cityId){
					...
					$cdsObj = $qb->select('cd')->from('Entity\CUOrderDetails','cd')->innerJoin('cd.cuso_id','Entity\CUSOrder')->innerJoin('Entity\CUSOrder.cu_id','Entity\CUnit')->where('Entity\CUnit.city_id=:cityId ')->setParameter('cityId',$cityId)->orderBy('cd.created_at','desc')->getQuery()->getResult();
					...
					foreach ($cdsObj as $key => $obj) {
						$processOrder = array();
							...
							$receiptNo  		= $placeOrderIdObj->getId();
							$status 			= $placeOrderIdObj->getOrderStatus();
								if(trim($placeOrderIdObj->getOrderStatus()=='CUAA' || trim($placeOrderIdObj->getOrderStatus())=='CUAPA')){
								$orderDate 			= strtotime($placeOrderIdObj->getOrderDate()->format('Y-m-d h:i:s'))*1000;
								//$orderDate 			= is_object($placeOrderIdObj->getOrderDate())?strtotime($placeOrderIdObj->getOrderDate()->format('Y-m-d H:i:s'))*1000:0;
								$pobj = $this->_em->getRepository('Entity\PlaceOrder')->findBy(array('order_id'=>$oid));
								if(count($pobj)){
									$details = array();
									$details['receiptNo'] = $receiptNo;
