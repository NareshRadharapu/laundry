   ...
   function __construct()
   {
			...
			if(is_object($data)){
				...
				if(property_exists($data,'storeId') && $storeId = $data->storeId)	{
					...
					try{
						...
						$storeInProcessing 	= array('SAA','PO','SAPI');
						$storeOutProcessing = array('SAAD','SAPAD','ORD','OPRD','DOA','DOPA');
					  	$storePending 		= array('STCU');
					  	$deliveryPending  = array('ORD','OPRD');

					  	$cuProcessing = array('CPBA','CUAA','CUAPA','HOLD','CUPA','CUPPA','CUTS','CUPSTS','HG-CUD','HG-CUP','HG-CUF');
						...
						foreach ($ordersArrayObj as $k => $oObj) {
							$pt[] = $oObj->getProcessDelayInDays();
								...
								if($oObj->getOrderStatus()==''){
									$newMobileOrders++;
								}else if(in_array($oObj->getOrderStatus(), $storeInProcessing)){
									$storeInProcessOrders++;
								}else if(in_array($oObj->getOrderStatus(), $storeOutProcessing)){
									$storeOutProcessOrders++;
								}else if(in_array($oObj->getOrderStatus(), array('STCU'))){
									$storePendingOrders++;
								}else if(in_array($oObj->getOrderStatus(), $deliveryPending)){
									$deliveryPendingOrders++;
								}else if(in_array($oObj->getOrderStatus(), $cuProcessing)){
									$cuProcessingOrders++;
								}else if(in_array($oObj->getOrderStatus(), $cuDeliveryPending)){
						...
						$result['store']['storePendingOrders'] = $storePendingOrders;
    ...
    public function superAdmindashBoard_post(){
    	$input = file_get_contents("php://input");
			$data = json_decode($input);
			if(is_object($data)){
				$fromDateObj = new \DateTime('- 1 days');
				if(property_exists($data,'fromDate') && $data->fromDate){
					$fromDate = $data->fromDate;
				}else{
					$fromDate = $fromDateObj->format('Y-m-d');	
				...
				$q = 'select s.name as store, s.isServiceTax as isServiceTax, sr.code as service, sum(po.icount) as totalItems, sum(po.ricount) as rItems, sum(po.cost) as totalCost,sum(po.reFund) as reFund, pi.order_id as orderId from place_order as po INNER JOIN place_order_ids as pi on po.order_id = pi.order_id left join areas as s on pi.store_id = s.area_id left join services as sr on po.service_id = sr.service_id WHERE pi.isDelete=0 and pi.isPackageOrder=0 and DATE(pi.orderDate) >="'.$fromDate.'" and DATE(pi.orderDate) <="'.$toDate.'"group by pi.store_id, po.service_id order by pi.store_id, sr.name';
					...
					foreach ($orderObj as $key => $obj) {
						...
						//$dstores[$store]['discountAmount'][$orderId] =  $discountAmount;
					}
					...
					foreach ($stores as $skey => $svalue) {
						...
						$ssFlats = array_merge($flat,array('discountAmount'=>array_sum($dstores[$skey]['discountAmount'])));
						$sc[$skey] = $ssFlats;
					}
				

				$discQ = 'select s.name as store , sum(pi.adminDiscountAmount) as discountAmount from  place_order_ids as pi left join areas as s on pi.store_id = s.area_id WHERE pi.isDelete=0 and DATE(pi.orderDate) >="'.$fromDate.'" and DATE(pi.orderDate) <="'.$toDate.'" group by pi.store_id order by pi.store_id';
					...
					foreach ($discObj as $key => $obj) {
						...
						$discountAmount = (int)$obj->discountAmount;
						$store 		= $obj->store;
						$diStores[$store] = array('discountAmount'=>$discountAmount);
					...
					foreach ($sc as $skey => $cvalue) {
						...
						if(array_key_exists($skey, $diStores)){
							...
							$flats = array_merge($cvalue,array('discountAmount'=>0));
						}
...
print_r($sc);
				die();		
				$transQ = 'select s.name as store , sum(th.paidAmount) as paidAmount from transactions_history as th INNER JOIN place_order_ids as pi on th.order_id = pi.order_id left join areas as s on pi.store_id = s.area_id WHERE pi.isDelete=0 and DATE(th.created_at) >="'.$fromDate.'" and DATE(th.created_at) <="'.$toDate.'" group by pi.store_id order by pi.store_id';
					...
					foreach ($transObj as $key => $obj) {
						...
						$store 		= $obj->store;
						$thStores[$store] = array('collections'=>$cost);
					...
					foreach ($sc as $skey => $cvalue) {
						...
						if(array_key_exists($skey, $thStores)){
							...
							$flats = array_merge($cvalue,array('collections'=>0));
						}
    ...
    public function dashBoardDetails_post(){
		...
		if(is_object($data)){
			if(property_exists($data,'rtype') && 
				...
				$storeInStatus = array('SAA','PO','SAPI',0,null);
				...
				$storeObj = $this->_em->find('Entity\Area',$storeId);
					...
					if(is_object($storeObj)){
						...
						switch ($type) {
								case 'lastDay':
								$ordersArrayObj 		= $storeObj->getLastDayPlaceOrderIds();
									break;
								case 'lastMonth':
									$ordersArrayObj 	= $storeObj->getLastMonthPlaceOrderIds();	
									break;
								case 'allRecords':
									$ordersArrayObj   			= $storeObj->getPlaceOrderIds();
									break;
						...
						foreach ($ordersArrayObj as $key => $orderObj) {
							...
							$order['rtype'] = $rtype;
							if($rtype == 'storeIn' && in_array($orderStatus, $storeInStatus)){
								$result['orders'][] = $order;
								
							}else
							if($rtype == 'storeOut' && in_array($orderStatus,$storeOutStatus)){
							
								$result['orders'][] = $order;	
							 
							}else
							if($rtype == 'storePending' && (
								in_array($orderStatus, $storeInStatus) || 
								in_array($orderStatus, $storeOutStatus))){
								$result['orders'][] = $order;	
								
							}
							else
							if($rtype == 'cuPending' && in_array($orderStatus, $cuStatus)){
								$result['orders'][] = $order;	
								
							}else
							if($rtype=='totalPending' && $orderStatus !='OD'){
								$result['orders'][] = $order;
								 
							}else if($rtype=='totalDelivery' && $orderStatus =='OD'){
								$result['orders'][] = $order;
								
							}else if($rtype=='totalOrders'){
								$result['orders'][] = $order;
								
							}else if($rtype=='balance' && (int)$orderObj->getBalanceAmount()>0){
								$result['orders'][] = $order;
							}else{
		...
		if(is_object($data) && property_exists($data,'inBarCode') && $data->inBarCode &&  property_exists($data,'message')){
			...
			try{
				...
				if($status=='returned' && $processObj->getItemStatus()!='returned' ){
					...
					$reFundAmt = $this->cbs->lostPlaceOrderItemIn($inBarCode);
		...
		if(is_object($data) && property_exists($data,'inBarCode') && $data->inBarCode){
			...
			//	log_message('error',$reFundAmt);
					...
					if($processObj->getItemStatus()=='returned'){
						$reFundAmt = $this->cbs->findPlaceOrderItemIn($inBarCode);
