    ...
    public function testApi_post(){
        ...
        if(is_object($data)){
          ...
          if(property_exists($data,'storeId') && $data->storeId)  {
            ...
            $closingReport['business'] = (int)($empObj[0]->business);
            $closingReport['pickups'] = (int)($empObj[0]->pickups);
            $closingReport['cash'] = $Cash;
            $closingReport['card'] = $DebitCard + $CreditCard;
            //$closingReport['CreditCard'] = $CreditCard;
            $closingReport['PayTM'] = $PayTM;
            $closingReport['Online'] = $Online;
            $closingReport['Cheque'] = $Cheque;
            $closingReport['cpba'] = (int)($cpbaObj[0]->cpba);
            $closingReport['stcu'] = (int)($stcuObj[0]->stcu);
            $closingReport['po'] = (int)($poObj[0]->po);
            $closingReport['saa'] = (int)($saaObj[0]->saa);
            $closingReport['change'] = (int)($chngObj[0]->chng);
            $closingReport['od'] = (int)($odObj[0]->od);
            $closingReport['expDel'] = (int)($expdelObj[0]->expDel);
            $closingReport['cupa'] = (int)($cupaObj[0]->cupa);
            $closingReport['penDel'] = (int)($pendelObj[0]->penDel);
            $closingReport['cRequests'] = (int)($crObj[0]->cRequests);
            $closingReport['mOrders'] = (int)($moObj[0]->mOrders);
          }
          // $message =['message'=>'Successfull' ,'closingReport'=>$closingReport];
          $this->response($closingReport, REST_Controller::HTTP_ACCEPTED);
    ...
    try{
        ...
        if($zone){
         ...
         if($zone == 1){
            ...
            $to = 'ambadaspk@gmail.com,radharapunaresh@gmail.com';
         }
...
public function storePendingBalance_post(){
 ...
 if(is_object($data)){
   ...
   if ($storeId) {
     ...
     $qpo = 'select c.cust_id as cId, c.firstname as firstname, c.lastname as lastname, c.mobile as mobile, c.wallet as walletAmount, pi.o_id as oid, pi.order_id as orderId, pi.orderDate as orderDate, pi.deliveryDate as deliveryDate, pi.balanceAmount as balanceAmount,pi.closingBalance as closingBalance, pi.poStatus as orderStatus, th.paymentFeedback as feedback from place_order_ids as pi inner join customers as c on pi.customer_id = c.cust_id left join transactions_history as th on th.order_id = pi.order_id WHERE pi.isDelete=0 and pi.isPackageOrder=0 and pi.balanceAmount>0 and pi.store_id="'.$storeId.'" and DATE(pi.orderDate) >="'.$fromDate.'" and DATE(pi.orderDate) <="'.$toDate.'" order by pi.orderDate desc' ;
   }
   else{
...
public function storePaidAmount_post(){
 ...
 if(is_object($data)){
   ...
   foreach ($ordersObj as $key => $orderObj) {
     ...
     $order['paymentDate'] = $orderObj->paymentDate;
    ...
    public function addCouponCode_post(){
    	...
    	if(is_object($data)){
    		if(property_exists($data,'code') && $data->code && 
    			...
    			property_exists($data,'count') && $data->count &&
    			property_exists($data,'minorderval') && $data->minorderval &&
			...
			if($orderId){
				try{
					...
					foreach ($placeOrderObj as $key => $value) {
						...
						$query = $this->db->query($mq);
						$itmObj = $query->result();
           // $itmsObj = $this->_em->getRepository('Entity\CatalogPrice')->findOneBy(array('item_id'=>$itmId,'service_id'=>$serviceId,'itype_id'=>$itypeId,'catalog_id'=>$catId));
					...
					$receipts['receipt']['totalAmount'] = number_format(($mainOrderObj->getTotalAmount()),2);
					$receipts['receipt']['totalItems'] = $mainOrderObj->getTotalItems();
