...
class CbsAdmin extends REST_Controller {
	function __construct(){
      ...
      public function cuStatuswiseReport_post(){
      	$this->_em = $this->doctrine->em;
      	...
      	if(is_object($data)){
      		...
      		$closingReport = array();
      		$this->load->database();
      		$cuaaq = "select count(os.CUAA) as cuaa, sum(pi.totalItems) as gCount,pi.store_id as storeId, os.CUAA as date, a.name as store from orderstatusdetails as os inner join place_order_ids as pi on pi.o_id = os.receiptNo inner join areas as a on a.area_id = pi.store_id where DATE(os.CUAA) = DATE('$date') group by pi.store_id";            
      		$query = $this->db->query($cuaaq);
      		$cuaaObj = $query->result(); 
      		$cupaq =  "select count(os.CUPA) as cupa, sum(pi.totalItems) as gCount,pi.store_id as storeId, os.CUPA as date, a.name as store from orderstatusdetails as os inner join place_order_ids as pi on pi.o_id = os.receiptNo left join areas as a on a.area_id = pi.store_id where DATE(os.CUPA) = DATE('$date') group by pi.store_id";             
      		$query = $this->db->query($cupaq);
      		$cupaObj = $query->result(); 
      		$totCuaaq = "select count(pi.o_id) as totCuaa, sum(pi.totalItems) as gCount, pi.store_id as storeId, a.name as store from place_order_ids as pi inner join areas as a on a.area_id = pi.store_id where pi.poStatus = 'CUAA' group by pi.store_id ";            
      		$query = $this->db->query($totCuaaq);
      		$totCuaaqObj = $query->result();            
      		$cuStatusReport = array();
      		$closingReport['cuaa'] = $cuaaObj; 
      		$closingReport['cupa'] = $cupaObj; 
      		$closingReport['totCuaa'] = $totCuaaqObj;
      		$cuStatusReport = $closingReport;            
          // $message =['message'=>'Successfull' ,'closingReport'=>$closingReport];
      		$this->response($cuStatusReport, REST_Controller::HTTP_ACCEPTED);
      	}else{
      		$message =[ 'message' => 'payload mistaken try again later'];
      		$this->response($message, REST_Controller::HTTP_NOT_ACCEPTABLE); 
      	}
      }
      public function cuStatuswiseOrders_post(){
      	$this->_em = $this->doctrine->em;
      	$qb = $this->_em->createQueryBuilder();
      	$input = file_get_contents("php://input");
      	$data = json_decode($input);
      	if(is_object($data)){
      		...
      		$this->load->database();
      		if($status =='received' ){
      			$mq = "select os.order_id as orderId, os.receiptNo as receiptId, pi.poStatus as status, pi.deliveryDate as dDate, pi.totalItems as totItems, a.name as store from orderstatusdetails as os inner join place_order_ids as pi on pi.o_id = os.receiptNo inner join areas as a on a.area_id = pi.store_id where DATE(os.CUAA) = DATE('$date') and pi.store_id = $storeId "; 
      		}else if ($status =='sent') {
      			$mq = "select os.order_id as orderId, os.receiptNo as receiptId, pi.poStatus as status, pi.deliveryDate as dDate, pi.totalItems as totItems, a.name as store from orderstatusdetails as os inner join place_order_ids as pi on pi.o_id = os.receiptNo inner join areas as a on a.area_id = pi.store_id where DATE(os.CUPA) = DATE('$date') and pi.store_id = $storeId ";
      		}else if ($status =='pending') {
      			$mq = "select pi.order_id as orderId, pi.o_id as receiptId, pi.poStatus as status, pi.deliveryDate as dDate, pi.totalItems as totItems, a.name as store from place_order_ids as pi inner join areas as a on a.area_id = pi.store_id where pi.poStatus = 'CUAA' and pi.store_id = $storeId ";
      		}
      		$query = $this->db->query($mq);
      		$StatuswiseOrdersObj = $query->result(); 
          // $message =['message'=>'Successfull' ,'closingReport'=>$closingReport];
      		$this->response($StatuswiseOrdersObj, REST_Controller::HTTP_ACCEPTED);
      	}else{
      		$message =[ 'message' => 'payload mistaken try again later'];
      		$this->response($message, REST_Controller::HTTP_NOT_ACCEPTABLE); 
      	}
      ...
      public function ordersDeliveryPrint_post(){
      	...
      	$data = json_decode($input);
      	if(is_object($data)){
      		if(property_exists($data, 'storeIds')){				
      			...
      			try{
      				...
      				if(sizeof($storeIds)){
      					foreach ($storeIds as $key => $sid) {
      						$mq = 'select s.area_id as storeId, s.name as store, s.mobile as storeContact, p.o_id as orderId, it.name as item, sc.name as service, po.icount as itemCount, po.ricount as rItemCount from place_order_ids as p inner join place_order as po on po.order_id = p.order_id left join cu_order_details as cud on cud.order_id = p.o_id left join areas as s on p.store_id = s.area_id left join items as it on po.item_id = it.item_id left join services as sc on po.service_id = sc.service_id where p.store_id = "'.$sid.'" and p.isDelete=0 and cud.cuso_id!="" and DATE(p.deliveryDate) ="'.$dDate.'" order by p.o_id, p.store_id';
      						$query = $this->db->query($mq);
      		...
      		if(property_exists($data, 'storeId') && property_exists($data, 'message')){
      			...
      			try{
    				...
    				//$customerObj = $qb->select('c')->from('Entity\Customer','c')->where('c.area_id=:storeId')->setParameter('storeId',$storeId)->getQuery()->getResult();
      				$customerObj = $qb->select('c')->from('Entity\Customer','c')->where('c.area_id=:storeId and c.status=:status')->setParameter('storeId',$storeId)->setParameter('status',1)->getQuery()->getResult(); 
      ...
      public function ordersRecordSearch_post(){
		...
		public function employees_get(){
			try{
				...
				e.mobile as mobile, 
				e.status as status,
				e.storeIp as storeIp, 
				e.loginTime as loginTime, 
				e.logoutTime as logoutTime, 
				r.role_name as role, 
				a.name as store from employees as e inner join roles as r on r.role_id = e.role_id left join areas as a on a.area_id = e.area_id order by e.emp_id ";
				$query = $this->db->query($mq);
				$empObj = $query->result();
      		//$this->response($empObj, REST_Controller::HTTP_OK);
			//$empObj = $this->_em->getRepository('Entity\Employee')->findAll();
				...
				foreach ($empObj as $key => $obj) {
					...
					$employee['store'] 	= $obj->store;
					$employee['status'] 	= (boolval($obj->status) ? 'true' : 'false')."\n";
					$employee['loginTime'] 	= $obj->loginTime;
					$employee['logoutTime'] = $obj->logoutTime;
					$employee['storeIp'] 	= $obj->storeIp;
		...
		public function vendorGroup_post(){
			$input = file_get_contents('php://input');
			$data = json_decode($input);
			if(is_object($data)){
				$id = 0;
				...
				try{
					if($id){
						$vd = $this->_em->find('Entity\VendorGroup',$id);
						if(!is_object($vd)){
							$vd = new Entity\VendorGroup();
						}
					}else{
						$vd = new Entity\VendorGroup();
					}
					...
					$this->_em->persist($vd);
					$this->_em->flush();
					$message = ['message'=>'successfully registerd an vendor.'];
					$this->set_response($message,REST_Controller::HTTP_CREATED);
				}catch(Exception $e){
					$message = ['message'=>$e->getMessage(),'status'=>500];
					$this->set_response($message,REST_Controller::HTTP_BAD_REQUEST);
		...
		public function editVendorGroup_post(){
			$input = file_get_contents('php://input');
			$data = json_decode($input);
			if(is_object($data)){
				try{
					$vdId = 0;
					if(property_exists($data,'vdId')){
						$vdId 		= $data->vdId;
						$vdObj = $this->_em->find('Entity\VendorGroup',$vdId);
						$vendor = array();
						$vendor['id'] 	= $vdObj->getId();
						$vendor['name'] 	= $vdObj->getName();
						$vendor['code'] 	= $vdObj->getCode();
						$vendor['status'] 	= $vdObj->getStatus();				
						$this->set_response($vendor,REST_Controller::HTTP_OK);
					}else{
						$message = ['message'=>'payload mistake','status'=>500];
						$this->set_response($message,REST_Controller::HTTP_BAD_REQUEST);
					}
		...
		public function vendorGroupStatus_post(){
			$input = file_get_contents("php://input");
			$data = json_decode($input);
			$vdId = $data;
			$this->_em = $this->doctrine->em;
			if((int)$vdId){
				$vendor = $this->_em->find('Entity\VendorGroup',(int)$vdId);
		...
		public function vendor_post(){
			...
			if(is_object($data)){
				...
				$vgroupId = '';
				if(property_exists($data, 'vgroupId')){
				...
				if(property_exists($data, 'discountPercent')){
					$discountPercent = stripcslashes($data->discountPercent);
				}
				$discountExpiry = '';
				if(property_exists($data, 'discountExpiry')){
					$discountExpiry = date('Y-m-d',strtotime($data->discountExpiry));
				}
				$commissionPercent = '';
				if(property_exists($data, 'commissionPercent')){
					$commissionPercent = stripcslashes($data->commissionPercent);
				}
				$commissionExpiry = '';
				if(property_exists($data, 'commissionExpiry')){
					$commissionExpiry = date('Y-m-d',strtotime($data->commissionExpiry));
				}
				try{
					if($id){
						$vd = $this->_em->find('Entity\Vendor',$id);
						if(!is_object($vd)){
							$vd = new Entity\Vendor();
						}
					}else{
						$vd = new Entity\Vendor();
					}
					$vd->setName($name);
					$vd->setCode($code);
					$vd->setVendorGroup($vgroupId);
					$vd->setEmail($email);
					$vd->setMobile($mobile);
					$vd->setDiscountPercent($discountPercent);
					$vd->setDiscountExpiry($discountExpiry);
					$vd->setCommissionPercent($commissionPercent);
					$vd->setCommissionExpiry($commissionExpiry); 
					$this->_em->persist($vd);
					$this->_em->flush();
					$message = ['message'=>'successfully registerd an vendor.'];
					$this->set_response($message,REST_Controller::HTTP_CREATED);
		...
		public function editVendor_post(){
			$input = file_get_contents('php://input');
			$data = json_decode($input);
			if(is_object($data)){
				try{
					...
					if(property_exists($data,'vdId')){
						$vdId 		= $data->vdId;
						$vdObj = $this->_em->find('Entity\Vendor',$vdId);
						$vendor = array();
						$vendor['id'] 	= $vdObj->getId();
						$vendor['name'] 	= $vdObj->getName();
						$vendor['code'] 	= $vdObj->getCode();
						$vendor['vgroupId'] = $vdObj->getVendorGroup();
						$vendor['email'] 	= $vdObj->getEmail();
						$vendor['mobile'] 	= $vdObj->getMobile();
						$vendor['discountPercent'] = $vdObj->getDiscountPercent();
						$vendor['discountExpiry'] = $vdObj->getDiscountExpiry()->format('d-m-Y');
						$vendor['commissionPercent'] = $vdObj->getCommissionPercent();
						$vendor['commissionExpiry'] = $vdObj->getCommissionExpiry()->format('d-m-Y');
						$this->set_response($vendor,REST_Controller::HTTP_OK);
					}else{
						$message = ['message'=>'payload mistake','status'=>500];
						$this->set_response($message,REST_Controller::HTTP_BAD_REQUEST);
		...
		public function vendorStatus_post(){
			...
			$this->_em = $this->doctrine->em;
			if((int)$vdId){
				$vendor = $this->_em->find('Entity\Vendor',(int)$vdId);
				$vendor->setStatus(!$vendor->getStatus());
				$this->_em->persist($vendor);
				$this->_em->flush(); die();
			}
		...
		public function vendorsReport_post(){
			$this->_em = $this->doctrine->em;
			$qb = $this->_em->createQueryBuilder();
			$input = file_get_contents("php://input");
			$data = json_decode($input);
			if(is_object($data)){
				...
				$closingReport = array();
				$this->load->database();
				$transQ = "select v.vendor_id as id, v.name as name, v.discountPercent as dPercent, v.discountExpiry as dExpiry, v.commissionPercent as cPercent, v.commissionExpiry as cExpiry, sum(pi.closingBalance) as ordersAmount from vendors as v inner join place_order_ids as pi on pi.vendorId = v.vendor_id  where pi.vendorId != '' and DATE(pi.orderDate) >='".$fromDate."' and DATE(pi.orderDate) <='".$toDate."' and pi.isDelete = 0 group by pi.vendorId";
				...
				foreach ($transObj as $key => $obj) {
					$vReport = array();
					$vReport['id']				= $obj->id;
					$vReport['name']			= $obj->name;
					$vReport['dPercent']		= $obj->dPercent;
					$vReport['dExpiry']			= $obj->dExpiry;
					$vReport['cPercent']		= $cPercent = $obj->cPercent;
					$vReport['cExpiry']			= $obj->cExpiry;
					$vReport['ordersAmount']	= $totAmount = number_format(($obj->ordersAmount),2,'.','');
					$afterGst				= $totAmount- ($totAmount*18)/100;
					$vReport['CommissionAmount']= number_format((($afterGst * $cPercent)/100),2,'.','');
					$vdReport[] = $vReport;
				}	
				$this->response($vdReport, REST_Controller::HTTP_ACCEPTED);
			}else{
				$message =[ 'message' => 'payload mistaken try again later'];
				$this->response($message, REST_Controller::HTTP_NOT_ACCEPTABLE); 
			}
		}
		public function vendorOrders_post(){
			$this->_em = $this->doctrine->em;
			$qb = $this->_em->createQueryBuilder();
			$input = file_get_contents("php://input");
			$data = json_decode($input);
			if(is_object($data)){	         
				$vId = 0;
				if(property_exists($data,'vId') && $data->vId)  {
					$vId = $data->vId; 	                 
				}
				$this->load->database();
				$transQ = "select pi.o_id as receiptNo, pi.orderDate as orderDate, c.firstname as firstname, c.lastname as lastname, pi.closingBalance as orderAmount from place_order_ids as pi inner join customers as c on c.cust_id = pi.customer_id where pi.vendorId = $vId and pi.isDelete = 0";
				$transQuery = $this->db->query($transQ);
				$transObj = $transQuery->result();
				$voReport = array();
				foreach ($transObj as $key => $obj) {
					$vReport = array();
					$vReport['receiptNo']		= $obj->receiptNo;
					$vReport['orderDate']		= date('d-M-Y',strtotime($obj->orderDate));
					$vReport['name']			= $obj->firstname.' '.$obj->lastname;
					$vReport['orderAmount']		= $obj->orderAmount;
					$voReport[] = $vReport;
				}	
				$this->response($voReport, REST_Controller::HTTP_ACCEPTED);
			}else{
				$message =[ 'message' => 'payload mistaken try again later'];
				$this->response($message, REST_Controller::HTTP_NOT_ACCEPTABLE); 
			}
