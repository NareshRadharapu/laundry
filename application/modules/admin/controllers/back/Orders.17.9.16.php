...
class Orders extends CI_Controller {
		...
		if(is_object($customerObj)){
			...
			try{
				...
				if(property_exists($result, 'sms') && $result->sms){
					...
					$this->benchmark->mark('stores');
					// echo time();
					// echo "ss";
					// $this->cbs->sendSMS($placeOrderId,'store');
					// echo "sms sent";
					// echo time();
					$this->benchmark->mark('storee');

            $fs =  $this->benchmark->elapsed_time('stores', 'storee');

            log_message('error',$fs);
	...
	// 	}
	// }on sendS
	public function sendSms($orderId,$type){
		$this->cbs->sendSMS($orderId,$type);
	}
	public function editMainOrder(){
		$input = file_get_contents('php://input'); 
		//$data = json_decode($input);
	...
	public function makepayment(){
		...
		try{
			if((int)$orderId){
				...
				$balanceAmount = $totalAmount-$paidAmount - $payingAmount - $wallet;
				...
				if($balanceAmount<0){
					...
					$order->getCustomerId()->setWallet(0);
				}
