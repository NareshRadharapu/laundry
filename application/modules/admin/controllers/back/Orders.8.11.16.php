			...
			try{
				...
				if(property_exists($result, 'addressId') && $addressId = $result->addressId){
					...
					if(is_object($address)){
						...
						$isServiceTax = $address->getAreaId()->getIsServiceTax();
					}
				}
				$config = $this->_em->find('Entity\Settings',1);	
				if(is_object($config) && $isServiceTax){
				...
				if(property_exists($result, 'adminDiscount') && $adminDiscount = $result->adminDiscount){
					...
					$adminDiscountPercent = round(($adminDiscount/$totalAmount)*100);

					$placeOrderId->setAdminDiscount((int)$adminDiscountPercent);
					
					$totalAmount = $totalAmount - $adminDiscount;
					
					$placeOrderId->setAdminDiscountAmount($adminDiscount);
				...
				$reFundAmount = $placeOrderId->getReFundAmount();
				if($paidAmount = $placeOrderId->getPaidAmount()){
					$balanceAmount = $totalAmount - (int)$paidAmount - $reFundAmount;
					...
					$closingAmount	= $totalAmount - $reFundAmount;
					$placeOrderId->setBalanceAmount($balanceAmount);	
					$placeOrderId->setClosingBalance($closingAmount);
				}	
