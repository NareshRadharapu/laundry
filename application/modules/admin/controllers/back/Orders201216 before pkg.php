			...
			try{
				...
				$totalItems = 0;
					...
					if(property_exists($myObj, 'qd') && $myObj->qd){
						$subTotal = $subTotal + $myObj->qdAmount;
						$placeOrderId->setQdAmount($myObj->qdAmount);
	...
	public function saveorder(){
		...
		if(is_object($customerObj)){
			...
			try{
				...
				if(property_exists($result, 'qdAmount')){
					$myObj->qdAmount	= (int)$result->qdAmount;
				}
				...
				if($customerObj->getPackageStatus()){
					...
					$packageFlag = true;
					$remainingPackageDetails = array();
					$remainingItemDetails = array();
					$remainingAddonDetails = array();
				}else{
					$packageFlag = false;
				}
					...
					foreach($result->temporders as $key=> $data){
						...
						if($packageFlag){
							...
							if($packageDetails['items'][$serviceId.'-'.$itypeId.'-'.$itemId]<$icount){
								$packageFlag = false;
							}else{
								//$remainingItemDetails[$serviceId.'-'.$itypeId.'-'.$itemId]= $icount - 

								$remainingPackageDetails['items'][$serviceId.'-'.$itypeId.'-'.$itemId] -=$icount;
								log_message('error','pakcage cost'.$remainingItemDetails['items'][$serviceId.'-'.$itypeId.'-'.$itemId]);
							}
						}
					...
					if($packageFlag){
						...
						$pd = json_encode($remainingPackageDetails);
						$customerObj->setPackageRemaingDetails($pd);
