...
class Customers extends CI_Controller {
	...
	public function customerlists(){
		...
		foreach ($customerObj as $key => $obj) {
			...
			$defaultDiscount 		= $obj->getDiscountPercent();
			if($defaultDiscount){
				$date = date('Y-m-d');
				$this->load->database();
				$dq = "select discountPercent from customers where DATE(discountExpiry) >= DATE('$date') and cust_id = '$id'";
				$query = $this->db->query($dq);
				$dDisObj = $query->result();
				if($dDisObj){							
					$customer['defaultDiscount'] = $dDisObj[0]->discountPercent;
				}else{
					$customer['defaultDiscount'] = 0;
				}
			}
			$vendorId 		= $obj->getVendorId();
			if($vendorId){
				$date = date('Y-m-d');
				$this->load->database();
				$dq = "select discountPercent from vendors where DATE(discountExpiry) >= DATE('$date') and vendor_id = '$vendorId' and status = '1'";
				$query = $this->db->query($dq);
				$disObj = $query->result();
				if($disObj){							
					$customer['vendorDiscount'] = $disObj[0]->discountPercent;
				}else{
					$customer['vendorDiscount'] = 0;
				}
			}
			$customers[] =  $customer;
		}
	...
	public function add(){
		...
		if(property_exists($data, 'discountExpiry')){
         ...
         }else{  // $discountExpiry = date('Y-m-d',strtotime($discountExpiry));
			$discountExpiry = '';
		}
	...
	try{
		...
		if($discountPercent){
			$customer->setDiscountPercent($discountPercent);
		}
		$customer->setDiscountExpiry($discountExpiry);
		if($aniversaryDate){
	...
	public function getaddress(){
		...
		if(is_object($customerObj)){
			...
			$defaultDiscount 			= $customerObj->getDiscountPercent();
			if($defaultDiscount){
				$date = date('Y-m-d');
				$this->load->database();
				$dq = "select discountPercent from customers where DATE(discountExpiry) >= DATE('$date') and cust_id = '$id'";
			...
			$vendorId 		= $customerObj->getVendorId();
			if($vendorId){
				$date = date('Y-m-d');
				$this->load->database();
				$dq = "select discountPercent from vendors where DATE(discountExpiry) >= DATE('$date') and vendor_id = '$vendorId' and status = '1'";
	...
	public function getbymobile(){
		...
		if(is_array($customerObj)){
			foreach ($customerObj as $key => $obj) {
				...
				// $customer['vendorId'] 		= $obj->getVendorId();
				$defaultDiscount 			= $obj->getDiscountPercent();
				if($defaultDiscount){
					$date = date('Y-m-d');
					$this->load->database();
	...
	public function customeredit(){
		...
		if(is_object($customerObj)){
			...
			if($dob)
				$customer['dob']			= $dob->format('d-m-Y');
			$aniversaryDate = $customerObj->getaniversaryDate();
			if($aniversaryDate)
				$customer['aniversaryDate']	= $aniversaryDate->format('d-m-Y');
			...
			$customer['discountPercent']= $customerObj->getDiscountPercent();
			$discountExpiry = $customerObj->getDiscountExpiry();
			if($discountExpiry)
				$customer['discountExpiry'] = $discountExpiry->format('d-m-Y');
			$customer['vendor']= $customerObj->getVendorId();
			$customerAddressObj 	= $customerObj->getCustomerAddress();
			if(is_object($customerAddressObj)){
				$customer['address'] 	= $customerAddressObj->getAddress();
				$customer['landmark'] 	= $customerAddressObj->getLandmark();
