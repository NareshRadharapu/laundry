...
class Visitor  extends CI_Controller {
	...
	public function listsz(){
		...
		$visitors = $qb->select('av','Entity\Visitor','Entity\Apartment')->from('Entity\ApartmentVisitor','av')->innerJoin('av.v_id','Entity\Visitor')->innerJoin('av.apt_id','Entity\Apartment')->getQuery()->getArrayResult();
		echo json_encode($visitors);
		die();
	}
	
	public function approvedlistsz(){
		...
		$visitors = $qb->select('av','Entity\Visitor','Entity\Apartment')->from('Entity\ApartmentVisitor','av')->innerJoin('av.v_id','Entity\Visitor')->innerJoin('av.apt_id','Entity\Apartment')->where('av.approved=:approved')->setParameter('approved',1)->getQuery()->getArrayResult();
		echo json_encode($visitors);
		die();
	}
