...
class Catalog extends CI_Controller {
	...
	public function lists(){
		...
		$catalogs = $qb->select('c')->from('Entity\Catalog','c')->addOrderBy('c.id','desc')->getQuery()->getArrayResult();
	...
	public function catalogservices(){
		...
		if($data){
			$catalogservices = $qb->select('c','Entity\Service')->from('Entity\CatalogPrice','c')->innerJoin('c.service_id','Entity\Service')->where('c.catalog_id=:catalog_id')->setParameter('catalog_id',$data)->getQuery()->getArrayResult();
		}
		echo json_encode($catalogservices);
		die();
