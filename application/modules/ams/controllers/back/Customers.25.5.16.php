...
class Customers extends CI_Controller {
	...
	public function getbymobile(){
		...
		$this->_em = $this->doctrine->em;
		$qb = $this->_em->createQueryBuilder();
		$customer = $qb->select('a','Entity\Area')->from('Entity\Customer','a')->innerJoin('a.area_id','Entity\Area')->where('a.mobile=:mobile')->setParameter('mobile',$id)->getQuery()->getArrayResult();
		if(sizeof($customer))
		echo json_encode($customer[0]);
		die();	
