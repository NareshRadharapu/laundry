...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function _detect_api_key()
    {
		...
		$authenticate = 'HTTP_' . strtoupper(str_replace('-', '_', 'authentication'));
		...
		//if((property_exists($result,'email') && property_exists($result,'password') && $result->email && $result->password && $this->input->server($authenticate)) || $this->input->server($authenticate)=='aparts'){
				return TRUE;	
		//}
        ...
        {	
			    ...
			    $user = $this->_em->getRepository('\Entity\Customer')->findOneBy(array('oauth_id'=>$key));
				if(is_object($user)){
					return TRUE;
				}else{
					return FALSE;
            ...
            $this->rest->key = $key; //$row->{$this->config->item('rest_key_column')};
