...
/* -------------------------------
 ...
 console.log(myHost);
 if (myHost == 'www.citybusinessservices.com')
  var ldh = 'http://www.citybusinessservices.com/laundry/';
else
  var ldh = 'http://192.168.0.110/laundry/';
...
colorAdminApp.controller('loginController', ['$rootScope', '$scope', '$http', '$location', 'ngNotify', '$state', function($rootScope, $scope, $http, $location, ngNotify, $state) {
  ...
  $scope.login = function(form, data) {
    if (form.$valid) {
      $http.post(ldh + 'store-authentication', data).then(function(response, status) {
        if (response.status == 202) {
          ngNotify.set(response.message);
          var roleName = response.data.employee.role;
          if (roleName == 'STORE_ADMIN') {
            $location.path('/app/dashboard').replace();
          } else {
            $location.path('/app/super-admin-dashboard').replace();
          }
          localStorage.setItem('myResource', response.data.resource);
          localStorage.removeItem('cu_user_obj');
          localStorage.removeItem('laundry_admin_user_obj');
          localStorage.setItem('laundry_admin_user_obj', angular.toJson(response.data))
        }
      }, function(error) {
  ...
  $scope.culogin = function(form, data) {
    if (form.$valid) {
      ...
      $http.post(ldh + 'cu-authentication', data).success(function(response, status) {
        if (status == 202) {
          ngNotify.set(response.message);
          localStorage.setItem('myResource', response.resource);
          localStorage.removeItem('cu_user_obj');
          localStorage.removeItem('laundry_admin_user_obj');
...
/*  Start ResourcesController */
colorAdminApp.controller('ResourcesController', ['$scope', '$http', 'pagination', 'ngNotify', 'NgTableParams', function($scope, $http, $pagination, ngNotify, NgTableParams) {
  $scope.curPage = 0;
  $scope.pageSize = 25;
  $scope.pagination = 0;
  $scope.heading = 'Add Itemtype ';
  $scope.resource = { resource: '', id: '', rlabel: '' };
  $scope.addResource = function() {
    $http.post(ldh + 'resource-add-edit', $scope.resource).success(function(response, status) {
      if ($scope.resource.id) {
        ngNotify.set('Your successfully updated ' + $scope.resource.rlabel + '');
      } else {
        ngNotify.set('Your successfully added ' + $scope.resource.rlabel + '');
      }
      $scope.resource = {};
      $scope.resource.apply;
      $scope.getResources();
    }).error(function(error) {
      console.log(error);
    });
  ...
  $scope.getResources = function() {
    $http.get(ldh + 'resource-list').success(function(response) {
      console.log(response);
      $scope.resources = response.resources;
      $scope.resources.apply;
      $scope.tableParams = new NgTableParams({ count: 25, sorting: { id: "dsc" } }, { counts: [25, 35, 50], dataset: response.resources });
    }).error(function(error) {
      console.log(error);
    });
  };
  $scope.getResources();
  $scope.statusResource = function(rid) {
    $http.post(ldh + 'resource-status', { 'rid': rid }).success(function(response, status) {
      ngNotify.set(response.message);
      $scope.getResources();
    }).error(function(error) {
      console.log(error);
  ...
  $scope.editResource = function(rid) {
    $http.post(ldh + 'single-resource', { 'rid': rid }).success(function(response, status) {
      $scope.resource.id = response.rid;
      $scope.resource.resource = response.resource;
      $scope.resource.rlabel = response.rlabel;
      $scope.resource.type = response.type;
      $scope.resource.apply;
    }).error(function(error) {
      console.log(error);
    });
...
colorAdminApp.controller('PermissionsController', ['$scope', '$http', 'pagination', 'ngNotify', 'NgTableParams', function($scope, $http, $pagination, ngNotify, NgTableParams) {
  $scope.curPage = 0;
  $scope.pageSize = 25;
  $scope.pagination = 0;
  $scope.heading = 'Add Itemtype ';
  $scope.resources = [];
  $scope.updatePermissionSet = function() {
    $http.post(ldh + 'permissions-update', $scope.resources).success(function(response, status) {
      ngNotify.set('Your successfully updated permission set');
      console.log(response);
      $scope.getResources();
    }).error(function(error) {
      console.log(error);
  ...
  $scope.heading = ' Resources ';
  $scope.getResources = function() {
    $http.get(ldh + 'permissions-list').success(function(response) {
      console.log(response);
      $scope.resources = response.resources;
      $scope.resources.apply;
    }).error(function(error) {
...
/*  Start CityController */
colorAdminApp.controller('testController', ['$rootScope', '$scope', '$http', 'pagination', 'ls', 'ngNotify', 'NgTableParams', function($rootScope, $scope, $http, $pagination, $ls, ngNotify, NgTableParams) {
  'use strict';
  $scope.curPage = 0;
  $scope.pageSize = 5;
  $scope.pagination = 0;
  ...
  $scope.filterOrdersBydate = function() {
    $scope.test.dDate = new Date($('#dDate').val());
    $scope.getCities();
    $scope.test.apply;
  };
  function formatGetCitiesResponse(resp){
    var aggregatedData= {};
    var keys = resp.revenue ? Object.keys(resp.revenue) : [];
    for(let key of keys){
      aggregatedData[key] = {};
      for(let key2 in resp.revenue[key]){
        aggregatedData[key][key2]={
          revenue: resp.revenue[key][key2]
        }
      }
      for(let key2 in resp.collection[key]){
        aggregatedData[key][key2]["collection"] = resp.collection[key][key2]
      }
      for(let key2 in resp.accounts[key]){
        aggregatedData[key][key2]["accounts"] =  resp.accounts[key][key2]
      }        
      for(let key2 in resp.bank[key]){
        aggregatedData[key][key2]["bank"] = resp.bank[key][key2]
      }
    }
  ...
  $scope.getCities = function() {
    $http.post(ldh + 'test-calender', $scope.test ).success(function(response, status) {
            //$scope.cities = response;
            $scope.calender = formatGetCitiesResponse(response);
            console.log($scope.calender); 
            $scope.tableParams = new NgTableParams({ count: 5, sorting: { date: "asc" } }, { counts: [5, 10, 25], dataset: $scope.calender });
            $scope.calender.apply;
          }).error(function(error) {
            console.log(error);
          });
        };
...
colorAdminApp.controller('AreaController', ['$scope', '$http', 'pagination', 'ngNotify', 'NgTableParams', function($scope, $http, $pagination, ngNotify, NgTableParams) {
  ...
  $scope.getAreas = function() {
    $http.get(ldh + 'admin/areas/lists').success(function(response, status) {
      ...
      console.log(response);
      $scope.tableParams = new NgTableParams({ count: 50, sorting: { id: "asc" } }, { counts: [50, 100, 250], dataset: response });
      $scope.areas.apply;
    }).error(function(error) {
      console.log(error);
    });
...
/*  Start CityController */
colorAdminApp.controller('HandoverReportController', ['$rootScope', '$scope', '$http',  'Auth', 'pagination', 'ls', 'ngNotify', 'NgTableParams', function($rootScope, $scope, $http,  Auth, $pagination, $ls, ngNotify, NgTableParams) {
  'use strict';
  $scope.curPage = 0;
  $scope.pageSize = 5;
  $scope.pagination = 0;
  ...
  $scope.filterOrdersBydate = function() {
    $scope.handoverData.dDate = new Date($('#dDate').val());
    $scope.getHandoverReport();
    $scope.handoverData.apply;
  };
  $scope.heading = ' Test ';
  $scope.cities = [];
  $scope.getHandoverReport = function() {
    $http.post(ldh + 'handover-report', $scope.handoverData ).success(function(response, status) {
      $scope.calender = formatGetCitiesResponse(response);
      console.log($scope.calender); 
      $scope.tableParams = new NgTableParams({ count: 5, sorting: { date: "asc" } }, { counts: [5, 10, 25], dataset: $scope.calender });
      $scope.calender.apply;
...
/*  Start StoreDailyReportController */
colorAdminApp.controller('StoreDailyReportController', ['$rootScope', '$scope', '$http', 'Auth', 'pagination', 'ls', 'ngNotify', 'NgTableParams', function($rootScope, $scope, $http, Auth, $pagination, $ls, ngNotify, NgTableParams) {
  'use strict';
  var areaId = Auth.getAreaId();
  console.log('storeId:', areaId);
  var userObj = localStorage.getItem('laundry_admin_user_obj');
  $scope.userRole = JSON.parse(userObj).employee.role;
  $scope.applyGlobalSearch = function() {
    $scope.tableParams.filter({ $: $scope.search });
  }
  $scope.report={storeId:areaId, business:'', cash:'', card:'', paytm:'',online:'', cheque:'', ironDone:'', ironPending:'', deliveries:'', pickups:'', stcu:'', pstcu:''}
  $scope.storeClosingReport = function() {
    $http.post(ldh + 'store-closing-report', $scope.report).success(function(response, status) {
      console.log(response);
      $scope.report = {};
      $scope.report.apply;
      if ($scope.report.business) {
        ngNotify.set('Your Report successfully updated ');
      } else {
        ngNotify.set('Your Report successfully added ');
      }
      $scope.report = {storeId:areaId};
      $scope.report.apply;
    }).error(function(error) { 
      console.log(error);
    });
  ...
  $scope.filterOrdersBydate = function() {
    ...
    $scope.payload.zone = $scope.zone;
    $scope.getClosingReport();
    $scope.payload.apply;
  };
  $scope.closingReport = [];
  $scope.payload = {date:'', zone:''}
  $scope.getClosingReport = function() {
    $http.post(ldh + 'store-daily-closing-report', $scope.payload).success(function(response, status) {
      $scope.closingReport = response;
           // console.log(response);
           $scope.closingReport.apply;
       ...
       $scope.sendReportEmail = function() {
        $scope.payloadEmail.date = new Date($('#date').val());
        $scope.payloadEmail.zone = $scope.zone;
        $scope.sendClosingReportEmail();
        $scope.payloadEmail.apply;
      };
      $scope.reportEmail = [];
      $scope.payloadEmail = {date:'', zone:''}
      $scope.sendClosingReportEmail = function() {
        $http.post(ldh + 'stores-daily-closing-report-email', $scope.payloadEmail).success(function(response, status) {
            //$scope.reportEmail = response;
            console.log(response);
           // $scope.reportEmail.apply;
           // $scope.tableParams = new NgTableParams({ count: 5, sorting: { id: "dsc" } }, { counts: [5, 10, 25], dataset: response });
         }).error(function(error) {
          console.log(error);
        });
       };
       $scope.report.business = 0;
       $scope.report.pickups = 0;
       $scope.report.stcu = 0;
       $scope.report.pstcu = 0;
       $scope.report.deliveries = 0;
       $scope.report.expDeliveries = 0;
       $scope.report.cash = 0;
       $scope.report.card = 0;
       $scope.report.paytm = 0;
       $scope.report.online = 0;
       $scope.report.cheque = 0;
       $scope.payloadReport = {date:'',storeId:areaId}
       $scope.getTodayReport = function() {
        $http.post(ldh + 'test-api', $scope.payloadReport).success(function(response, status) {
          $scope.report.business = parseInt(response.business);
          $scope.report.pickups = parseInt(response.pickups);
          $scope.report.stcu = parseInt(response.cpba);
          $scope.report.pstcu = parseInt(response.change)+parseInt(response.stcu)+parseInt(response.saa)+parseInt(response.po);
          $scope.report.deliveries = parseInt(response.od);
          $scope.report.expDeliveries = parseInt(response.expDel);
            // $scope.report.cash = parseInt(response.collection[0].Cash);
            // $scope.report.card = parseInt(response.collection[0].DebitCard)+parseInt(response.collection[0].CreditCard);
            // $scope.report.paytm = parseInt(response.collection[0].PayTM);
            // $scope.report.online = parseInt(response.collection[0].Online);
            // $scope.report.cheque = parseInt(response.collection[0].Cheque);
            console.log($scope.report.cheque);           
          }).error(function(error) {
            console.log(error);
          });
        };
...
/*  Start OrderwiseStatusController */
colorAdminApp.controller('OrderwiseStatusController', ['$rootScope', '$scope', '$http', 'Auth', 'pagination', 'ls', 'ngNotify', 'NgTableParams', function($rootScope, $scope, $http, Auth, $pagination, $ls, ngNotify, NgTableParams) {
  'use strict';
  var areaId = Auth.getAreaId();
    //console.log('storeId:', areaId);
    var userObj = localStorage.getItem('laundry_admin_user_obj');
    ...
    getAreasList();
    $scope.ordersdata = {storeId:areaId,fromDate:'',toDate:''};
    var monthsList = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
    $scope.fromDateSelected =  (new Date().getMonth() + 1) + '/' + new Date().getDate() + '/' +(new Date().getFullYear());
    $scope.dateSelected =  (new Date().getMonth()+ 1) + '/' +new Date().getDate() + '/' + (new Date().getFullYear());
    ...
    $scope.filterOrdersBydate= function(){
      ...
      $scope.getOrderwiseStatusDetails();
      $scope.ordersdata.apply;                    
    };
    $scope.getOrderwiseStatusDetails = function(){
      $http.post(ldh + 'orderwise-status-report',$scope.ordersdata).success(function(response, status){
        console.log("response", response);
        $scope.tableParams = new NgTableParams({ count: 100, sorting: { id: "asc" } }, { counts: [200, 500, 1000], dataset: response });
      }).error(function(err) {
        console.log("err", err);
      });
    };
...
/*--- StoresAndTargetsController ----  */
colorAdminApp.controller('StoresAndTargetsController', ['$scope', '$http', 'pagination', 'ngNotify', 'NgTableParams', function($scope, $http, $pagination, ngNotify, NgTableParams) {
  'use strict';
  $scope.curPage = 0;
  $scope.pageSize = 5;
  $scope.pagination = 0;
  $scope.heading = 'Stores And Targets';
  $scope.storeTarget = {};
  $scope.storesList = [];
  $("#targetMonth").datepicker({}).on('changeDate', function(ev) {
    $scope.storeTarget.targetMonth = $('#targetMonth').val();
  });
  $http.get(ldh + 'api-areas').success(function(response, status) {
    $scope.storesList = response;
    console.log("stores", response);
    $scope.storesList.apply;
  }).error(function(error) {
    console.log(error);
  });
  $scope.addStoreTarget = function() {
    $http.post(ldh + 'store-add-edit-target', $scope.storeTarget).success(function(response, status) {
      if ($scope.storeTarget.id)
        ngNotify.set('Your successfully updated ');
      else
        ngNotify.set('Your successfully added ');
      $scope.storeTarget = {};
      $scope.storeTarget.apply;
      $scope.getStoreTargets();
    }).error(function(error) {
      console.log(error);
    });
  };
  $scope.singleStoreTarget = function($a) {
    $http.post(ldh + 'store-target', { 'stId': $a }).success(function(response, status) {
      $scope.storeTarget.stId = response.stId;
      $scope.storeTarget.ordersTarget = response.ordersTarget;
      $scope.storeTarget.revenueTarget = response.revenueTarget;
      $scope.storeTarget.targetMonth = response.targetMonth;
      $scope.storeTarget.storeId = response.storeId;
    }).error(function(error) {
      console.log(error);
    });
  };
  $scope.heading = ' Stores And Targets ';
  $scope.statusStoreTarget = function($a) {
    $http.post(ldh + 'store-target-status', $a).success(function(response, status) {
      $scope.getStoreTargets();
      ngNotify.set('Status successfully changed.');
    }).error(function(error) {
      console.log(error);
  ...
  $scope.storeTargets = [];
  $scope.getStoreTargets = function() {
    $http.get(ldh + 'stores-targets').success(function(response, status) {
      console.log(response);
      $scope.storeTargets.apply;
      $scope.tableParams = new NgTableParams({ count: 5, sorting: { id: "dsc" } }, { counts: [5, 10, 25], dataset: response.storesTargets });
    }).error(function(error) {
      console.log(error);
    });
  };
...
colorAdminApp.controller('StoreLoginLogoutController', ['$scope', '$http', 'Auth', 'pagination', 'ngNotify', 'NgTableParams', function($scope, $http, Auth, $pagination, ngNotify, NgTableParams) {
  $scope.applyGlobalSearch = function() {
    $scope.tableParams.filter({ $: $scope.search });
  }
  $scope.logindata = {date:''};
  var monthsList = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
  $scope.fromDateSelected =  (new Date().getMonth() + 1) + '/' + new Date().getDate() + '/' +(new Date().getFullYear());
  $scope.dateSelected =  (new Date().getMonth()+ 1) + '/' +new Date().getDate() + '/' + (new Date().getFullYear());
  $('#datepicker-default-from').datepicker({
  ...
  $scope.filterOrdersBydate= function(){
    $scope.logindata.date = new Date($('#datepicker-default-from').val());;
    $scope.getloginHistory();
    $scope.logindata.apply;     
  };
  $scope.storeLogins = {};
  $scope.getloginHistory = function() {
    $http.post(ldh + 'store-login-logout-history', $scope.logindata).success(function(response, status) {
      console.log("response", response);
      $scope.storeLogins = response;
      $scope.storeLogins.apply;
      $scope.tableParams = new NgTableParams({ count: 100, sorting: { id: "asc" } }, { counts: [200, 500, 1000], dataset: response });
    }).error(function(err) {
      console.log(err);
    });
...
colorAdminApp.controller('StoreDeliveryOrdersController', ['$scope', '$http', 'Auth', 'pagination', 'ngNotify', 'NgTableParams', function($scope, $http, Auth, $pagination, ngNotify, NgTableParams) {
  var areaId = Auth.getAreaId();
  var userObj = localStorage.getItem('laundry_admin_user_obj');
  $scope.userRole = JSON.parse(userObj).employee.role;
  $scope.applyGlobalSearch = function() {
  ...
  getAreasList();
  $scope.ordersdata = { storeId:areaId, serviceId:'', dDate: '' };
  var monthsList = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
  $scope.fromDateSelected = (new Date().getMonth() + 1) + '/' + new Date().getDate() + '/' + (new Date().getFullYear());
  $scope.dateSelected = (new Date().getMonth() + 1) + '/' + new Date().getDate() + '/' + (new Date().getFullYear());
  $('#dDate').datepicker({
  ...
  $scope.filterOrdersBydate = function() {
    ...
    $scope.ordersdata.serviceId = $scope.serviceId;
  ...
  $scope.getDeliveryOrders = function() {
    $http.post(ldh + 'store-day-to-delivery-orders', $scope.ordersdata).success(function(response, status) {
      console.log("response", response);
      ...
      $scope.tableParams = new NgTableParams({ count: 200, sorting: { id: "asc" } }, { counts: [200, 500, 1000], dataset: res });
    }).error(function(err) {
      console.log(err);
    });
  }
...
colorAdminApp.controller('IndividualUserController', ['$scope', '$http', '$stateParams', 'NgTableParams', 'ngNotify', function($scope, $http, $stateParams, NgTableParams, ngNotify) {
  ...
  $scope.viewOrders = function($o) {
    $http.post(ldh + 'customer-orders', { 'customerId': $o }).success(function(response, status) {
      ...
      $scope.orders.apply;
      if ($scope.booleanVal) {
        $scope.tableParams = new NgTableParams({ count: 25, sorting: { id: "dsc" } }, { counts: [50, 100, 250], dataset: response.orders });
    ...
    $http.post(ldh + 'transaction-history', { 'customerId': $o }).success(function(response, status) {
      if (!$scope.booleanVal) {
        $scope.tableParams = new NgTableParams({ count: 25, sorting: { id: "dsc" } }, { counts: [50, 100, 250], dataset: response.transactions });
        $scope.tableParams.apply;
      }
      console.log(response);
    }).error(function(error) {
...
colorAdminApp.controller('CustomerPackageController', ['$scope', '$http', '$stateParams', 'NgTableParams', 'ngNotify', 'Auth',
  function($scope, $http, $stateParams, NgTableParams, ngNotify, Auth) {
    $scope.orders = { orders: [], details: '' };
    $scope.booleanVal = false;
    var storeId = Auth.getAreaId();
    $scope.viewOrders = function() {
      $http.post(ldh + 'customer-package-orders', { 'storeId': storeId }).success(function(response, status) {
        $scope.orders = response;
        $scope.orders.apply;
    ...
    $scope.viewOrders();
    $scope.customerPackage = { customerId: '', agentId: '' };
    $scope.orderToogle = function(cid) {
            //console.log(cid);
            $scope.customerPackage.customerId = cid;
            return $scope.booleanVal = !$scope.booleanVal;
          }
          $scope.pickupBoys = [];
          $scope.getPickupBoys = function($id) {
            $http.post(ldh + 'store-pickup-boys', { areaId: $id }).success(function(response) {
              ...
              $scope.pickupBoys.unshift({ pbId: '', name: '-select-' });
              $scope.pickupBoys.apply;
            }).error(function(err) {
              console.log(err);
            });
          };
          $scope.getPickupBoys(storeId);
          $scope.assignAgent = function() {
            $http.post(ldh + 'package-assign-to-pickupboy', $scope.customerPackage).success(function(response, status) {
              ngNotify.set(response.message, { type: 'success', duration: 2000 });
              $scope.booleanVal = false;
            }).error(function(err) {
              console.log(err);
...
colorAdminApp.controller('StorePendingBalanceController', ['$scope', '$http', 'Auth', 'pagination', 'ngNotify', 'NgTableParams', function($scope, $http, $Auth, $pagination, ngNotify, NgTableParams) {
  var areaId = $Auth.getAreaId();
  $scope.applyGlobalSearch = function() {
    $scope.tableParams.filter({ $: $scope.search });
  }
  ...
  $('#datepicker-default-to').datepicker({
    ...
    $scope.filterOrdersBydate = function() {
      ...
      $scope.getPendingBalanceOrders();
      $scope.ordersdata.apply;
    };
    $scope.orders = [];
    $scope.getPendingBalanceOrders = function() {
      $http.post(ldh + 'store-pending-balance', $scope.ordersdata).success(function(response, status) {
        console.log("response", response);
        var balanceAmount = 0;
        var res = response.orders;
        for (var i in res) {
          balanceAmount += parseInt(res[i].balanceAmount);
        }
        $scope.totalBalanceAmount = balanceAmount;
        $scope.totalBalanceAmount.apply;
        $scope.tableParams = new NgTableParams({ count: 2000, sorting: { id: "asc" } }, { counts: [25, 50, 100], dataset: res });
      }).error(function(err) {
        console.log("err", err);
      });
    };
    $scope.payment = {};
    $scope.paymentToggleModal = function($o, $b, $w) {
      $scope.payment.oid = $o;
      $scope.payment.balanceAmount = $b;
      $scope.payment.walletAmount = $w;
...
colorAdminApp.controller('StoreCollectionController', ['$scope', '$http', 'Auth', 'pagination', 'ngNotify', 'NgTableParams', function($scope, $http, $Auth, $pagination, ngNotify, NgTableParams) {
  var areaId = $Auth.getAreaId();
  $scope.applyGlobalSearch = function() {
    $scope.tableParams.filter({ $: $scope.search });
  }
  ...
  getAreasList();
  $scope.ordersdata = { storeId: areaId, pType: '', fromDate: '', toDate: '' };
  var monthsList = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
  $scope.fromDateSelected = (new Date().getMonth() + 1) + '/' + new Date().getDate() + '/' + (new Date().getFullYear());
  $scope.dateSelected = (new Date().getMonth() + 1) + '/' + new Date().getDate() + '/' + (new Date().getFullYear());
  $('#datepicker-default-from').datepicker({
  ...
  $scope.filterOrdersBydate = function() {
    $scope.ordersdata.fromDate = new Date($('#datepicker-default-from').val());;
    $scope.ordersdata.toDate = new Date($('#datepicker-default-to').val());
    $scope.ordersdata.pType = $scope.pType;
    if (areaId == 0) {
      $scope.ordersdata.storeId = $scope.storeId;
    }
    $scope.getpaidAmountOrders();
    $scope.ordersdata.apply;
  };
  $scope.applyGlobalSearch = function() {
    $scope.tableParams.filter({ $: $scope.search });
  ...
  $scope.orders = [];
  $scope.getpaidAmountOrders = function() {
    $http.post(ldh + 'store-paid-amount', $scope.ordersdata).success(function(response, status) {
      console.log("response", response);
      var paidAmount = 0;
      var res = response.orders;
      for (var i in res) {
        paidAmount += parseInt(res[i].paidAmount);
      }
      $scope.totalPaidAmount = paidAmount;
      $scope.totalPaidAmount.apply;
      $scope.tableParams = new NgTableParams({ count: 100, sorting: { id: "asc" } }, { counts: [100, 500, 1000], dataset: res });
    }).error(function(err) {
      console.log("err", err);
    });
  };
...
colorAdminApp.controller('StoreTransactionsController', ['$scope', '$http', 'Auth', 'pagination', 'ngNotify', 'NgTableParams', function($scope, $http, $Auth, $pagination, ngNotify, NgTableParams) {
  $scope.applyGlobalSearch = function() {
    $scope.tableParams.filter({ $: $scope.search });
  }
  var userObj = localStorage.getItem('laundry_admin_user_obj');
  $scope.userRole = JSON.parse(userObj).employee.role;
  $scope.userStore = JSON.parse(userObj).employee.store;
  $scope.today      = new Date();
  console.log($scope.userStore);
  var areaId = $Auth.getAreaId();
  var empId = $Auth.getEmpId();
  console.log($Auth.getEmpId(),'Auth.getEmpId()');
  console.log($Auth.getAreaId(),'Auth.getAreaId()');
  function getAreasList() {
    $http.get(ldh + 'api-areas')
    .then(function(response, status) {
      $scope.areasList = response.data;
  ...
  getAreasList();
  function getCollectionBoysList() {
    $http.get(ldh + 'api-cboys')
    .then(function(response, status) {
      $scope.cboysList = response.data;
      console.log($scope.cboysList);
    }, function(error) {
      console.log(error);
    });
  }
  getCollectionBoysList();
  $scope.cboyId   ='';
  $scope.account  ='';
  $scope.cbOrder = { 'orders': [], 'empId': empId, 'cboyId':'', 'account':'' };
  $scope.assignToCb = function() {
    $scope.cbOrder.cboyId = $scope.cboyId;
    $scope.cbOrder.account   = $scope.account;
        //console.log($scope.cbOrder);
        var falseCount = 0;
        if ($scope.cbOrder.orders.length > 0) {
          angular.forEach($scope.cbOrder.orders, function(val, key) {
            if (val === false) {
              falseCount += 1;
            }
          });
          if (falseCount === $scope.cbOrder.orders.length) {
            ngNotify.set('Please select an order to send it to CBoy', { type: 'error' });
          } else {
            $http.post(ldh + 'cboy-assign', $scope.cbOrder).success(function(response, status) {
             $scope.cbOrder = { 'orders': [], 'empId': empId, 'cboyId':'', 'account':'' };                    
             ngNotify.set(response.message, { status: 'success' });
             $scope.cboyId   ='';
             $scope.account  ='';                    
             $scope.getStoreTransactions();
             $scope.getCboyTransactions();
           }).error(function(err) {
            console.log(err);
          });
         }
       } else if ($scope.cbOrder.orders.length === 0) {
        ngNotify.set('Please select an order to send it to CBoy', { type: 'error' });
      }
    }
    $scope.applyGlobalSearch = function() {
    ...
    $scope.orders = [];
    $scope.getStoreTransactions = function() {
      $http.post(ldh + 'store-transactions', $scope.ordersdata).success(function(response, status) {
        console.log("response", response);
        $scope.tHistory = response;
        ...
        $scope.totalPaidAmount = paidAmount;
        $scope.totalPaidAmount.apply;
        $scope.tableParams = new NgTableParams({ page: 1, total: 1, count: 5000, sorting: { id: "desc" } }, { counts: [], dataset: res });
      }).error(function(err) {
        console.log("err", err);
      });
    };
    /* Store Cboy Transactions */
    $scope.ordersCdata = { storeId: areaId, pType: '', fromDate: '', toDate: '' };
    var monthsList = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
    $scope.fromDateSelected1 = (new Date().getMonth() + 1) + '/' + new Date().getDate() + '/' + (new Date().getFullYear());
    $scope.dateSelected1 = (new Date().getMonth() + 1) + '/' + new Date().getDate() + '/' + (new Date().getFullYear());
    $('#datepicker-default-from1').datepicker({
      setDate: new Date(),
      todayHighlight: true,
      format: 'dd/MM/yyyy'
    });
    $('#datepicker-default-to1').datepicker({
      setDate: new Date(),
      todayHighlight: true,
      format: 'dd/MM/yyyy'
    });
    $scope.totalCollectAmount = 0;
    $scope.filterOrdersBydate1 = function() {
      $scope.ordersCdata.fromDate = new Date($('#datepicker-default-from1').val());;
      $scope.ordersCdata.toDate = new Date($('#datepicker-default-to1').val());
      $scope.ordersCdata.pType = $scope.pType;
      if (areaId == 0) {
        $scope.ordersCdata.storeId = $scope.storeId;
      }
      $scope.getCboyTransactions();
      $scope.ordersCdata.apply;
    };
    $scope.orders = [];
    $scope.cboyHistory = '';
    $scope.getCboyTransactions = function() {
      $http.post(ldh + 'storeCboy-transactions', $scope.ordersCdata).success(function(response, status) {
        console.log("response", response);
        $scope.cboyHistory = response;
        var paidAmount = 0;
        var res = response;
        for (var i in res) {
          paidAmount += parseInt(res[i].paidAmount);
        }
        $scope.totalCollectAmount = paidAmount;
        $scope.totalCollectAmount.apply;
           // $scope.tableParams = new NgTableParams({ page: 1, total: 1, count: 5000, sorting: { id: "desc" } }, { counts: [], dataset: res });
         }).error(function(err) {
          console.log("err", err);
        });
       };
       $scope.unAssignToCb = function($c) {
        $http.post(ldh + 'cboy-unassign', $c).success(function(response, status) {
          console.log("response", response);
          $scope.getStoreTransactions();
          $scope.getCboyTransactions();
        }).error(function(err) {
...
colorAdminApp.controller('CboyTransactionsController', ['$scope', '$http', 'Auth', 'pagination', 'ngNotify', 'NgTableParams', function($scope, $http, $Auth, $pagination, ngNotify, NgTableParams) {
  $scope.applyGlobalSearch = function() {
    $scope.tableParams.filter({ $: $scope.search });
  }
  var userObj = localStorage.getItem('laundry_admin_user_obj');
  ...
  getAreasList();
  function getAccountantsList() {
    $http.get(ldh + 'api-accountants')
    .then(function(response, status) {
      $scope.accountantsList = response.data;
      console.log($scope.accountantsList);
    }, function(error) {
      console.log(error);
    });
  }
  getAccountantsList();
  $scope.acId  ='';
  $scope.acOrder = { 'orders': [], 'acId': '', 'cboyId':empId };
  $scope.assignToAc = function() {
    $scope.acOrder.acId = $scope.acId;
        //console.log($scope.acOrder);
        var falseCount = 0;
        if ($scope.acOrder.orders.length > 0) {
          angular.forEach($scope.acOrder.orders, function(val, key) {
            if (val === false) {
              falseCount += 1;
            }
          });
          if (falseCount === $scope.acOrder.orders.length) {
            ngNotify.set('Please select an order to send it to A/C', { type: 'error' });
          } else {
               // console.log($scope.cbOrder, '$scope.cbOrder');
               $http.post(ldh + 'accountant-assign', $scope.acOrder).success(function(response, status) {
                    //console.log(response);
                    $scope.acOrder = { 'orders': [], 'acId': '', 'cboyId':''};
                    ngNotify.set(response.message, { status: 'success' });
                    $scope.acId  ='';
                    $scope.getCboyTransactions();
                    $scope.getAccTransactions();
                  }).error(function(err) {
                    console.log(err);
                  });
                }
              } else if ($scope.acOrder.orders.length === 0) {
                ngNotify.set('Please select an order to send it to A/C', { type: 'error' });
              }
            }
            /* Store Cboy Transactions */
            $scope.ordersdata = { cboyId: empId, storeId: '', pType: '', fromDate: '', toDate: '' };
            var monthsList = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
            $scope.fromDateSelected = (new Date().getMonth() + 1) + '/' + new Date().getDate() + '/' + (new Date().getFullYear());
            $scope.dateSelected = (new Date().getMonth() + 1) + '/' + new Date().getDate() + '/' + (new Date().getFullYear());
            $('#datepicker-default-from').datepicker({
            ...
            $scope.orders = [];
            $scope.getCboyTransactions = function() {
              $http.post(ldh + 'cboy-transactions', $scope.ordersdata).success(function(response, status) {
                console.log("response", response);
                $scope.cHistory = response;
                $scope.paymentType = response.paymentType;
                var paidAmount = 0;
                var res = response;
                for (var i in res) {
            ...
            /* Store Acc Transactions */
            $scope.ordersCdata = { storeId: areaId, pType: '', fromDate: '', toDate: '', cboyId: empId };
            var monthsList = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
            $scope.fromDateSelected1 = (new Date().getMonth() + 1) + '/' + new Date().getDate() + '/' + (new Date().getFullYear());
            $scope.dateSelected1 = (new Date().getMonth() + 1) + '/' + new Date().getDate() + '/' + (new Date().getFullYear());
            $('#datepicker-default-from1').datepicker({
            ...
            $scope.filterOrdersBydate1 = function() {
              ...
              $scope.ordersCdata.pType = $scope.pType;
              if (areaId == 0) {
                $scope.ordersCdata.storeId = $scope.storeId1;
              }
              $scope.getAccTransactions();
              $scope.ordersCdata.apply;
            };
            $scope.orders = [];
            $scope.getAccTransactions = function() {
              $http.post(ldh + 'cboyAcc-transactions', $scope.ordersCdata).success(function(response, status) {
                console.log("response", response);
                $scope.accHistory = response;
                var paidAmount = 0;
                var res = response;
                for (var i in res) {
                  paidAmount += parseInt(res[i].paidAmount);
       ...
       $scope.unAssignToAc = function($c) {
        $http.post(ldh + 'accountant-unassign', $c).success(function(response, status) {
          console.log("response", response);
          $scope.getCboyTransactions();
          $scope.getAccTransactions();
        }).error(function(err) {
...
colorAdminApp.controller('AccountantTransactionsController', ['$scope', '$http', 'Auth', 'pagination', 'ngNotify', 'NgTableParams', function($scope, $http, $Auth, $pagination, ngNotify, NgTableParams) {
  $scope.applyGlobalSearch = function() {
    $scope.tableParams.filter({ $: $scope.search });
  }
  var userObj = localStorage.getItem('laundry_admin_user_obj');
  ...
  getAccountantsList();
  $scope.account  ='';
  $scope.acOrder = { 'orders': [], 'account': '', 'empId':empId };
  $scope.assignToBank = function() {
    $scope.acOrder.account = $scope.account;
        //console.log($scope.acOrder);
        var falseCount = 0;
        if ($scope.acOrder.orders.length > 0) {
          angular.forEach($scope.acOrder.orders, function(val, key) {
          ...
          if (falseCount === $scope.acOrder.orders.length) {
            ngNotify.set('Please select an order to send it to Bank', { type: 'error' });
          } else {
               // console.log($scope.cbOrder, '$scope.cbOrder');
               $http.post(ldh + 'bank-assign', $scope.acOrder).success(function(response, status) {
                    //console.log(response);
                    $scope.acOrder = { 'orders': [], 'account': '', 'empId':empId};
                    ngNotify.set(response.message, { status: 'success' });
                    $scope.account  ='';
                    $scope.getAccountantTransactions();
                  }).error(function(err) {
                    console.log(err);
                  });
                }
            ...
            $scope.ordersdata = { acId: empId, storeId: '', pType: '', fromDate: '', toDate: '' };
            var monthsList = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
            $scope.fromDateSelected = (new Date().getMonth() + 1) + '/' + new Date().getDate() + '/' + (new Date().getFullYear());
            $scope.dateSelected = (new Date().getMonth() + 1) + '/' + new Date().getDate() + '/' + (new Date().getFullYear());
            $('#datepicker-default-from').datepicker({
            ...
            $scope.orders = [];
            $scope.getAccountantTransactions = function() {
              $http.post(ldh + 'accountant-transactions', $scope.ordersdata).success(function(response, status) {
                console.log("response", response);
                $scope.accHistory = response;
                $scope.paymentType = response.paymentType;
                var paidAmount = 0;
    ...
    $http.post(ldh + 'admin/catalog/edit', $c).success(function(response, status) {
        ...
        $scope.getCatalogs = function() {
          $http.get(ldh + 'admin/catalog/lists').success(function(response, status) {
            ...
            console.log(response);
            $scope.catalogs.apply;
            $scope.tableParams = new NgTableParams({ count: 25, sorting: { id: "asc" } }, { counts: [25, 100, 250], dataset: response });
          }).error(function(error) {
            console.log(error);
          });
        };
      ...
      $scope.editCatalogItem = function($c) {
        ...
        $scope.getCatalogItems = function() {
          $ls.getCatalogItems().then(function(data) {
            ...
            $scope.tableParams = new NgTableParams({ count: 200, sorting: { id: "dsc" } }, { counts: [200, 300, 500, 1000], dataset: data.data });
          });
        }
        $scope.statusArea = function($a) {
          $http.post(ldh + 'admin/areas/status', $a).success(function(response, status) {
...
colorAdminApp.controller('CatalogServiceController', ['$scope', '$http', '$stateParams', 'NgTableParams', function($scope, $http, $stateParams, NgTableParams) {
  ...
  $http.post(ldh + 'admin/catalog/catalogservices', $c).success(function(response, status) {
    $scope.catalogservices = response;
    $scope.catalogservices.apply;
    $scope.tableParams = new NgTableParams({ count: 200, sorting: { id: "asc" } }, { counts: [200, 300, 500], dataset: response });
  }).error(function(error) {
    console.log(error);
  });
  $scope.viewServiceItem = function($a, $b) {
    ...
    $http.post(ldh + 'admin/catalog/serviceitems', $scope.serviceitem).success(function(response, status) {
      ...
      $scope.tableParams = new NgTableParams({ count: 200, sorting: { id: "dsc" } }, { counts: [200, 300, 500], dataset: response });
            //console.log(response);
          }).error(function(error) {
            console.log(error);
          });
...
colorAdminApp.controller('CustomerListController', ['$scope', '$http', 'pagination', 'ngNotify', 'NgTableParams', 'Auth', function($scope, $http, $pagination, ngNotify, NgTableParams, Auth) {
  $scope.curPage = 0;
  $scope.pageSize = 5;
  $scope.pagination = 0;
  ...
  $scope.userObj = angular.fromJson(localStorage.getItem('laundry_admin_user_obj'));
    ...
    $scope.getVendor = function($it) {
      console.log($it)
      $scope.code = $it;
      return $http.post(ldh + 'api-vendor',{code: $scope.code}).then(function(response) {
        $scope.vendorData = response.data.name;
        $scope.customer.vendor = response.data.id;
        $scope.customer.vendor.apply;            
      });
    };
    ...
    $scope.customer = { firstname: '', id: '', area: areaId, dob:'', aniversaryDate:'', discountExpiry:'' };
    var checkin = $('#dob').datepicker({
      ...
      autoclose: true,
      onSelect: function() {
        $scope.customer.dob = $('#dob').val();
      }
    });
    var checkin = $('#aniversaryDate').datepicker({
      setDate: new Date(),
      todayHighlight: true,
      dateFormat: 'dd-mm-yy',
        //  endDate: '+0d',
      autoclose: true,
      onSelect: function() {
        $scope.customer.aniversaryDate = $('#aniversaryDate').val();
      }
    });
    var checkin = $('#discountExpiry').datepicker({
      setDate: new Date(),
      todayHighlight: true,
      dateFormat: 'dd-mm-yy',
        //  endDate: '+0d',
      autoclose: true,
      onSelect: function() {
        $scope.customer.discountExpiry = $('#discountExpiry').val();
      }
    });
    ...
    $scope.editCustomer = function($a) {
      $http.post(ldh + 'admin/customers/customeredit', $a).success(function(response, status) {
          ...
          $scope.customer.landmark = response.landmark;
          $scope.customer.pincode = response.pincode;
          $scope.customer.lattitude = response.lattitude;
          $scope.customer.longitude = response.longitude;
          $scope.customer.dob = response.dob;
          $scope.customer.aniversaryDate = response.aniversaryDate;
          $scope.customer.isProblematic = response.isProblematic;
          $scope.customer.isStarch = response.isStarch;
          $scope.customer.isPerfume = response.isPerfume;
          $scope.customer.discountPercent = response.discountPercent;
          $scope.customer.discountExpiry = response.discountExpiry;
          $scope.customer.id = response.id;
          $scope.customer.area = response.area_id;
          $scope.customer.vendor = response.vendor;
          $scope.customer.userEdit = 1;
          console.log(response);
        }).error(function(error) {
          console.log(error);
        });
      }
      $scope.customerPackage = { 'customerId': '', 'packageId': '', 'packageList': [] };
      $scope.customerPackageFlag = false;
      $scope.customerPackageModal = function(cid) {
        $scope.customerPackageFlag = !$scope.customerPackageFlag;
        $scope.customerPackage.customerId = cid;
        $scope.customerPackage.apply;
      }
      $scope.getPackages = function() {
        $http.post(ldh + 'current-packages', {}).then(function(data) {
          $scope.customerPackage.packageList = data.data.packages;
        }, function(err) {
          console.log(err);
        });
      };
      ...
      $scope.addPackage = function() {
        $http.post(ldh + 'add-customer-package', $scope.customerPackage).success(function(response, status) {
          $scope.customerPackageFlag = false;
      ...
      if ($scope.userObj && $scope.userObj.employee) {
        userRole = $scope.userObj.employee.role;
      }
      $scope.getCustomers = function() {
        $http.post(ldh + 'admin/customers/customerlists', { storeId: areaId, role: userRole }).success(function(response, status) {
          $scope.customers = response;
          console.log(response);
          $scope.customers.apply;
          $scope.tableParams = new NgTableParams({ count: 25, sorting: { id: "dsc" } }, { counts: [25, 50, 75], dataset: response });
        }).error(function(error) {
          console.log(error);
        });
      }
    $scope.customer.userEdit = 0;
    $scope.getCustomers();
  }]);
...
colorAdminApp.controller('customerEnquiryController', ['$scope', '$http', 'ls', 'ngNotify', '$rootScope', '$state', '$timeout', function($scope, $http, $ls, ngNotify, $rootScope, $state, $timeout) {
  'use strict';
  $scope.customer = { mobile: '', areaId: '', area: '', requestDateTime: '' };
  ...
  $scope.customerNotFound = false;
    ...
    $scope.checkExist = function() {
      $http.post(ldh + 'admin/customers/getbymobile', $scope.customer.mobile).success(function(response, status) {
        console.log(response);
        $rootScope.areaId = response.area;
        if (!response.id) {
          $scope.customerFound = false;
          ...
          $scope.customerNotFound = false;
            ...
            //console.log("response.serviceWiseRevenue",response.serviceWiseRevenue);
            var serviceWiseRevenue = '';
            if (response.serviceWiseRevenue)
              serviceWiseRevenue = angular.fromJson(response.serviceWiseRevenue);
            //console.log("response.serviceWiseRevenue",serviceWiseRevenue);
            var serviceWiseItems = '';
            if (response.serviceWiseItems)
              serviceWiseItems = angular.fromJson(response.serviceWiseItems);
            $scope.customer = response;
            $scope.customer.serviceWiseRevenue = serviceWiseRevenue;
            $scope.customer.serviceWiseItems = serviceWiseItems;
            $scope.customer.apply;
          }).error(function(error) {
            console.log(error);
            $scope.customerFound = false;
        ...
        $scope.customerToggleFlag = false;
        $scope.customerToggle = function() {
          $scope.customerToggleFlag = !$scope.customerToggleFlag;
        };
        $scope.addCustomer = function() {
          console.log('Customer form submited');
          $http.post(ldh + 'admin/customers/customerstore', $scope.customer).success(function(response, status) {
        ...
        // Changes by Raj eshwar 25-08-2016 START
        $('#datetimepicker1').datepicker({
          todayHighlight: true,
          format: 'dd-mm-yyyy'
        });
        $('#datetimepicker1').on("changeDate", function() {
          $scope.customer.requestDateTime = $('#requestDateTime').val();
        });
        // $('#datetimepicker1').on("dp.change", function(e) {
        //  console.log('changeDate', e.date);
        //  $scope.customer.requestDateTime = e.date
        // });
        // Changes by Raj eshwar 25-08-2016 END
      ...
      $scope.makeRequest = function() {
        ...
        $http.post(ldh + 'customer-add-request', $scope.customer).success(function(response, status) {
          $scope.cRequest = false;
          console.log(response);
          $timeout(function() { $state.go('app.customerRequests'); }, 500);
        }).error(function(error) {
          console.log(error);
        });
      };
...
/*******************************************************************/
colorAdminApp.controller('PlaceOrderController', ['$scope', '$rootScope', '$http', 'ls', 'ngNotify', '$stateParams', '$state', 'Auth', function($scope, $rootScope, $http, $ls, ngNotify, $stateParams, $state, Auth) {
  'use strict';
  $scope.heading = ' Place Order Form';
  /* Today Login Checking Start */
  $scope.userObj = angular.fromJson(localStorage.getItem('laundry_admin_user_obj'));
  $scope.userRole = '';
  if ($scope.userObj && $scope.userObj.employee) {
    $scope.userId = $scope.userObj.employee.id;
  }
  console.log('checking');
  var emp_id = $scope.userId;
  var checkin = new Date();
  $scope.payloads = { empId: emp_id, loginTime: checkin};
  $scope.check = function() {
    $http.post(ldh + 'store-check', $scope.payloads).success(function(response, status) {
  ...
  $scope.check();
  /* Today Login Checking End */
  $scope.subtotal = 0;
  var orderSaved = false;
  $scope.placeorder = { id: '', customerId: '', orderId: '', service: '', itemtype: '', pitem: '', icount: '', addons: [] };
  $scope.mainorder = { orderId: '', 'isDiscount': true, couponCode: '', customerId: '', temporders: [], orderDate: '', deliveryDate: '', adminDiscount: 0, addressId: 1, totalAmount: '', qd: false, qdAmount: 0, qdPercent: 0, sms: false, ordSrc: '' };
  ...
  $scope.userRole = JSON.parse(userObj).employee.role;
    ...
    $('#datepicker-default-delivery').datepicker({
      todayHighlight: true,
      startDate: '+1d',
      endDate: '+15d',
      format: 'dd-mm-yyyy',
    ...
    $('#datepicker-default-delivery').on("changeDate", function(e) {
      console.log('changeDate', e.date);
      $scope.mainorder.deliveryDate = $('#datepicker-default-delivery').val();
    });
    $scope.discountAndQuickDelivery = true;
    $scope.mainorder.qd = false;
    // if($scope.mainorder.adminDiscountAmount.length > 0  || $scope.mainorder.qdAmount.length > 0 ) {
    //  $scope.discountAndQuickDelivery = false;
    // }
    $scope.$watch('mainorder.qd', function() {
      if ($scope.mainorder.qd == false) {
        console.log($scope.subtotal);
        $scope.mainorder.qdPercent = 0;
        $scope.quickCalculation();
      }
        // $scope.mainorder.qdPercent = 10;
      });
    $scope.quickCalculation = function() {
      $scope.discountAndQuickDelivery = false;
      console.log("mainorder", $scope.mainorder);
      var qdPercent = Number($scope.mainorder.qdPercent || 0);
      console.log(qdPercent);
      var subtotal = Number($scope.subtotal || 0);
      console.log(subtotal);
      var discount = Number($scope.mainorder.adminDiscountAmount || 0);
      console.log(discount);
      $scope.subtotal1 = subtotal + subtotal * (qdPercent / 100) - discount;
      console.log($scope.subtotal1);
      if ($scope.addresses.servicetax == true) {
        $scope.mainorder.totalAmount = $scope.subtotal1 + $scope.subtotal1 * (14.5 / 100);
      } else {
        $scope.mainorder.totalAmount = $scope.subtotal1;
      }
    }
    $scope.discountType = function(t) {
      if (t == 'd') {
        $scope.mainorder.isDiscount = true;
        $scope.mainorder.isCoupon = false;
      } else {
        $scope.mainorder.isCoupon = true;
        $scope.mainorder.isDiscount = false;
      }
      $scope.mainorder.apply;
        ...
        $http.post(ldh + 'admin/orders/getpreorder', $scope.placeorder).success(function(response, status) {
          ...
          console.log($scope.discountAndQuickDelivery);
          $scope.discountAndQuickDelivery = true;
          $scope.subtotal = 0;
          ...
          $scope.quickCalculation();
            // $scope.mainorder.addressId = $rootScope.areaId;
            // console.log($scope.mainorder.addressId);
          }).error(function(error) {
            console.log(error);
          });
        };
        $scope.getPreOrder1 = function($customerId) {
          if ($customerId != '') {
            $scope.placeorder.customerId = $customerId;
          }
        //console.log($scope.placeorder);
      ...
      if ($stateParams.id) {
        ...
        $http.post(ldh + 'admin/orders/editMainOrder', $scope.orderId).success(function(response, status) {
          ...
          $scope.mainorder.ordSrc = response.ordSrc;
          ...
          $scope.mainorder.couponCode = response.couponCode;
          $scope.mainorder.adminDiscountAmount = Number(response.adminDiscountAmount || 0);
          $scope.mainorder.orderDate = response.orderDate;
          $scope.mainorder.qd = response.qd;
          $scope.mainorder.qdPercent = Number(response.qdPercent || 0);
          $scope.subtotal = response.subTotal - $scope.mainorder.qdAmount + $scope.mainorder.adminDiscountAmount;
          console.log($scope.subtotal);
          console.log($scope.mainorder.adminDiscountAmount);
            // $scope.mainorder.deliveryDate       = response.deliveryDate;
            $scope.mainorder.orderId = response.orderId;
            if (response.totalAmount - response.subTotal > 0) {
              $scope.addresses.servicetax = true
            }
            $scope.getPreOrder1(response.customerId);
            $scope.quickCalculation();
            // $scope.mainorder.qdAmount      = response.qdAmount;
            // $scope.subtotal = $scope.subtotal1 = response.subTotal;
            // Check  on monday
            //  if($scope.discountAndQuickDelivery == true) {
            //     $scope.subtotal = response.subTotal;
            // } else if ($scope.discountAndQuickDelivery == false) {
            //  $scope.subtotal = response.subTotal;
            //  var abc = $scope.subtotal;
            //  $scope.subtotal1 = abc;       
            // }
            //  $scope.mainorder.totalAmount = response.totalAmount;
            $scope.getAddress();
          }).error(function(error) {
            console.log(error);
          });
      ...
      $scope.services = [];
      $scope.itemTypes = [];
      $scope.areaId = Auth.getAreaId();
      $scope.services = function() {
        $http.post(ldh + 'api-services',{areaId: $scope.areaId}).success(function(response, status) {
          $scope.services = response;
          $scope.services.apply;
          console.log(response);
        }).error(function(error) {
    ...
    $scope.savePreOrder = function() {
      ...
      $scope.saveMainOrder = function() {
        ...
        //}
        // if($scope.mainorder.deliveryDate == '' || !$scope.mainorder.deliveryDate){
        //  ngNotify.set('Please select delivery date',  {type: 'error',duration: 5000});
        // }else{
          if($scope.mainorder.ordSrc == '' || !$scope.mainorder.ordSrc){
           ngNotify.set('Please select Order Source Storevisit / Pickupboy',  {type: 'error',duration: 5000});
         }else{      
    ...
    */
    ...
    $http.post(ldh + 'admin/orders/saveorder', $scope.mainorder).success(function(response, status) {
      ...
      ngNotify.set(' main order completed..... ');
            /*if($scope.mainorder.sms){
              $http.post(ldh+'api-sms',{'isStore':true}).success(function(response, status){
              }).error(function(err){
                console.log(err);
              })
            }*/
      ...
      $scope.assignable = function(order_id) {
        ...
        //$('.selectdatepicker').selectdatepicker('render');
...
colorAdminApp.controller('CouponsController', ['$scope', '$state', '$http', 'NgTableParams', 'ngNotify',
  'Auth', '$rootScope',
  function($scope, $state, $http, NgTableParams, ngNotify, Auth, $rootScope) {
    $scope.coupon = { status: 1, startDate:'', expDate:'', code:'',cost:'',count:'',minorderval:''};
    $scope.coupons = [];
    var checkin = $('#startDate').datepicker({
      ...
      autoclose: true,
      onSelect: function() {
        $scope.coupon.startDate = $('#startDate').val();
      }
    });
    var checkin = $('#expDate').datepicker({
      ...
      dateFormat: 'dd-mm-yy',
            ...
            autoclose: true,
            onSelect: function() {
              $scope.coupon.expDate = $('#expDate').val();
            }
          });
    $scope.addCoupon = function() {
      $http.post(ldh + 'add-coupon-code', $scope.coupon).success(function(response, status) {
        $scope.getCoupons();
        if ($scope.coupon.id)
          ngNotify.set('Your successfully updated ' + $scope.coupon.code + '', { html: true });
        else
          ngNotify.set('Your successfully added ' + $scope.coupon.code + '', { html: true });
        $scope.coupon = {};
        $scope.coupon.apply;
      }).error(function(error) {
        console.log(error);
    ...
    $scope.singleCoupon = function($a) {
      $http.post(ldh + 'coupon-code', { 'couponId': $a }).success(function(response, status) {
                $scope.coupon.code = response.code; // apartment name populate
                $scope.coupon.id = response.id;
                $scope.coupon.cost = response.cost;
                $scope.coupon.count = response.count;
                $scope.coupon.minorderval = response.minorderval;
                $scope.coupon.startDate = response.startDate;
                $scope.coupon.expDate = response.expDate;
                $scope.coupon.status = response.status;
              }).error(function(error) {
                console.log(error);
              });
            };
            $scope.heading = ' Coupon ';
            $scope.getCoupons = function() {
              $http.get(ldh + 'coupon-codes').success(function(response, status) {
                $scope.coupons = response;
                console.log(response);
                $scope.coupons.apply;
                $scope.tableParams = new NgTableParams({ count: 10, sorting: { id: "dsc" } }, { counts: [25, 50, 100], dataset: response.coupons });
              }).error(function(error) {
                console.log(error);
              });
            };
...
colorAdminApp.controller('couponsUsedController', ['$rootScope', '$scope', '$http', 'pagination', 'ls', 'ngNotify', 'NgTableParams', function($rootScope, $scope, $http, $pagination, $ls, ngNotify, NgTableParams) {
  'use strict';
  $scope.applyGlobalSearch = function() {
    $scope.tableParams.filter({ $: $scope.search });
  }
  $scope.cDetails = { fdate: '', tdate: '' };
  var monthsList = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
  $scope.fromDateSelected = (new Date().getMonth() + 1) + '/' + new Date().getDate() + '/' + (new Date().getFullYear());
  $scope.dateSelected = (new Date().getMonth() + 1) + '/' + new Date().getDate() + '/' + (new Date().getFullYear());
  $('#datepicker-default-from').datepicker({
  ...
  $scope.filterOrdersBydate = function() {
    $scope.cDetails.fdate = new Date($('#datepicker-default-from').val());;
    $scope.cDetails.tdate = new Date($('#datepicker-default-to').val());
    $scope.getCouponDetails();
    $scope.cDetails.apply;
  };
  $scope.heading = ' Coupon Details ';
  $scope.getCouponDetails = function() {
    $http.post(ldh + 'test-api', $scope.cDetails ).success(function(response, status) {
      $scope.couponDetails = response;
      $scope.tableParams = new NgTableParams({ count: 25, sorting: { id: "dsc" } }, { counts: [5, 10, 25], dataset: response });
      $scope.couponDetails.apply;
    }).error(function(error) {
      console.log(error);
    });
  };
...
colorAdminApp.controller('PlaceOrdersController', ['$scope', '$state', '$http', 'NgTableParams', 'ngNotify',
  'Auth', '$window', '$interval', '$rootScope', '$location',
  function($scope, $state, $http, NgTableParams, ngNotify, Auth, $window, $interval, $rootScope, $location) {
    /* Today Login Checking Start */
    $scope.userObj = angular.fromJson(localStorage.getItem('laundry_admin_user_obj'));
    $scope.userRole = '';
    if ($scope.userObj && $scope.userObj.employee) {
    ...
    $scope.pageSize = 5;
    $scope.pagination = 0;
    $scope.payment = { oid: '', totalAmount: '', balanceAmount: '', payingAmount: '', walletAmount: '', dueAmount: '' };
    $scope.showModal = false;
    $scope.selectedDate = {};
    var d = new Date();
    ...
    var areaId = Auth.getAreaId();
    $scope.orderPayLoad = { 'storeId': areaId, 'fromDate': '', 'toDate': '' }
    var userObj = localStorage.getItem('laundry_admin_user_obj');
    $scope.userRole = JSON.parse(userObj).employee.role;
    $scope.orderStatusObj = {
      ...
      message: '',
      orderId: '',
            statusList: ['SAA', 'SAPI', 'OD'], //, 'PO','OPr', 'RSCU'
            messagesList: { 'SAPI': ['Count  mismatch', 'Damage found'] },
            filteredMessages: []
          };
        ...
        $scope.cuOrder = { 'orders': [], 'cueId': '', 'empId': empId, 'cuId': '' };
        $scope.placeOrderRequest = { orderId: '', pickupBoyId: '', deliveryBoyId: '', type: '' };
        var monthsList = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
        // $scope.dateSelected = new Date().getDate() + '/' + monthsList[new Date().getMonth()] + '/' + new Date().getFullYear();
        $scope.dateSelected = new Date();
        // changes by ankit
        var checkin = $('#datepicker-default-from').datepicker({
          setDate: new Date(),
          todayHighlight: true,
          format: 'dd-mm-yyyy',
          autoclose: true
          ...
          $('#datepicker-default-to')[0].focus();
        });
        var checkout = $('#datepicker-default-to').datepicker({
          setDate: new Date(),
          todayHighlight: true,
          format: 'dd-mm-yyyy',
            //    endDate: '+0d',
        ...
        $scope.filterOrdersBydate = function() {
            ...
            var fromDate = new Date($("#datepicker-default-from").datepicker("getDate")).getTime();
            var toDate = new Date($("#datepicker-default-to").datepicker("getDate")).getTime();
            console.log(fromDate);
            console.log(toDate);
            $scope.orderPayLoad.fromDate = fromDate;
            $scope.orderPayLoad.toDate = toDate;
            if (areaId == 0) {
              $scope.orderPayLoad.storeId = $scope.storeId;
            }
            //filterOrderItems(fromDate,toDate);
            console.log("$scope.orderPayLoad", $scope.orderPayLoad)
            $scope.getOrdersList();
          };
          $scope.passignable = function(order_id) {
            var orderId = order_id;
            var indigator = orderId.split("-");
            indigator = indigator[2];
            if (indigator == 'M') {
              return true;
            } else {
              return false;
            ...
            //refresh function ankith
            if ($state.current.name === "app.placeorders") {
        ...
        // };
        // $scope.refreshed();
        $scope.dassignable = function(order_id, status) {
          var orderId = order_id;
          var indigator = orderId.split("-");
          indigator = indigator[2];
          if (status == 'ORD' || status == 'OPRD') {
            return true;
          } else {
            return false;
          }
        ...
        $scope.toggleModal = function($o, $t, $b, $paidAmount, $walletAmount, $dueAmount, adminDiscountAmount, adminDiscount) {
          $scope.payment.oid = $o;
          ...
          $scope.payment.dueAmount = $dueAmount;
          $scope.payment.adminDiscountAmount = adminDiscountAmount;
          $scope.payment.adminDiscount = adminDiscount;
          $scope.payment.discountValue = adminDiscountAmount + ' ' + adminDiscount + '%';
          $scope.showModal = !$scope.showModal;
        };
        /*** Make Payment **/
        $scope.makePayment = function() {
        ...
        /*** order delete ***/
        $scope.deleteOrder = function(oid, t) {
          if (confirm("Are you want to delete '" + oid + "'")) {
            $scope.currentItems = [];
            $http.post(ldh + 'admin/orders/deleteOrder', { orderId: oid, status: t }).then(function(response, status) {
              $scope.getOrdersList();
            });
          }
        };
        function filterOrderItems(fromDate, toDate, areaCode) {
          ...
          angular.forEach($scope.allItems, function(val, key) {
            ...
            newToDate.setDate(toDate.getDate() + 1);
                ...
                $scope.orderCount += 1;
                $scope.totalQd  += parseFloat(val.qdAmount);
                $scope.totalIcount += (val.totalItems);
                ...
                $scope.reFundAmount += parseFloat(val.reFundAmount);
                $scope.closingAmount += parseFloat(val.closingAmount);
              });
          if (areaCode) {
            var areaItems = [];
            $scope.orderCount = 0;
            ...
            angular.forEach($scope.currentItems, function(val, key) {
              if (val.order_id.indexOf(areaCode) >= 0) {
                areaItems.push(val);
              }
            });
            $scope.currentItems = areaItems;
          } else {
            console.log("this is area code  else");
          }
        ...
        $scope.getOrdersList = function() {
          $http.post(ldh + 'admin/orders/orderslist', $scope.orderPayLoad).then(function(response, status) {
            console.log("response orders list", response.data);
            $scope.allItems = response.data;
                // console.log($scope.selectedDate);
                var totalRevenue = 0;
                var res = response.data;
                for(var i in res){
                  totalRevenue +=parseInt(res[i].closingAmount);
                }
                $scope.totalRevenue = totalRevenue;
                $scope.totalRevenue.apply;
                console.log('revenue:', $scope.totalRevenue);
        ...
        $scope.assignToCu = function() {
          ...
          var falseCount = 0;
          if ($scope.cuOrder.orders.length > 0) {
            angular.forEach($scope.cuOrder.orders, function(val, key) {
              if (val === false) {
                falseCount += 1;
              }
            });
            if (falseCount === $scope.cuOrder.orders.length) {
              ngNotify.set('Please select an order to send it to CU', { type: 'error' });
            } else {
              console.log($scope.cuOrder, '$scope.cuOrder');
              $http.post(ldh + 'cu-place-order', $scope.cuOrder).success(function(response, status) {
                console.log(response);
                $scope.cuOrder = { 'empId': empId, 'orders': [] };
                ngNotify.set(response.message, { status: 'success' });
                $scope.getOrdersList();
              }).error(function(err) {
                console.log(err);
          ...
          } else if ($scope.cuOrder.orders.length === 0) {
            ngNotify.set('Please select an order to send it to CU', { type: 'error' });
          }
        }
        $scope.openStatusModal = function(order) {
          ...
          if (!order.deliveryDate) {
            ngNotify.set("order must have delivery date", { status: 'warn' });
            $location.path('/app/placeorder/orderid/' + order.order_id).replace();
            return;
          }
          $scope.selectedOrderItem = order;
          $scope.orderStatusObj.orderId = order.order_id;
          $scope.showOrderStatusModal = true;
        }
        $scope.changeOrderStatus = function() {
          $http.post(ldh + 'store-order-change', $scope.orderStatusObj).success(function(response, status) {
            $scope.showOrderStatusModal = false;
            ngNotify.set('Status modified successfully', { status: 'success' });
            $http.post(ldh + 'admin/orders/orderslist', areaId)
            .then(function(response, status) {
              $scope.orderStatusObj.status = '';
              $scope.allItems = response.data;
              $scope.filterOrdersBydate();
            });
          }).error(function(err) {
            ngNotify.set('Error while modying the status', { status: 'error' });
            console.log(err);
          });
        }
        $scope.orderStatusSelected = function(status) {
          console.log(status);
          $scope.orderStatusObj.filteredMessages = $scope.orderStatusObj.messagesList[status];
        };
        $scope.applyGlobalSearch = function() {
          $scope.tableParams.filter({ $: $scope.search });
        }
        ...
        $scope.deliveryBoyToggle = false;
        $scope.deliveryToggle = function($a) {
          $scope.placeOrderRequest.orderId = $a;
          $scope.deliveryBoyToggle = !$scope.deliveryBoyToggle;
        }
        $scope.pickupBoys = [];
        $scope.getPickupBoys = function($id) {
          $http.post(ldh + 'store-pickup-boys', { areaId: $id }).success(function(response) {
...
/** Deleted Orders Controller */
colorAdminApp.controller('DeletedOrdersController', ['$scope', '$state', '$http', 'NgTableParams', 'ngNotify', 'Auth', function($scope, $state, $http, NgTableParams, ngNotify, Auth) {
  $scope.order = { orderId: '', customerId: '' };
  $scope.placeOrders = [];
  $scope.curPage = 0;
  $scope.pageSize = 5;
  ...
  $scope.orderStatusObj = {
    ...
    message: '',
    orderId: '',
        statusList: ['SAA', 'SAPI'], //, 'PO','OPr', 'RSCU'
        messagesList: { 'SAPI': ['Count  mismatch', 'Damage found'] },
        filteredMessages: []
      };
    //var cbsLocal  = localStorage.getItem('laundry_admin_user_obj');
    ...
    $scope.assignable = function(order_id) {
      ...
      if (indigator == 'M') {
        ...
        /*$scope.dassignable= function(order_id,dstatus){
          var orderId=order_id;
          var indigator=orderId.split("-");
          indigator=indigator[2];
          return false;
          if(indigator=='M' && dstatus=='SADA'){
            return true;
          } else{
            return false;
          }
        ...
        $scope.toggleModal = function($o, $t, $b, $paidAmount, $walletAmount, adminDiscountAmount, adminDiscount) {
          $scope.payment.oid = $o;
          $scope.payment.totalAmount = $t;
          $scope.payment.paidAmount = $paidAmount;
          $scope.payment.balanceAmount = $b;
        ...
        /*** order delete ***/
        $scope.deleteOrder = function(oid, t) {
          if (confirm("Are you want to undo '" + oid + "'")) {
            $http.post(ldh + 'admin/orders/deleteOrder', { orderId: oid }).then(function(response, status) {
              $scope.getOrdersList();
            });
          }
        };
        ...
        /*** end make payment ***/
        $scope.getOrdersList = function() {
          $http.post(ldh + 'admin/orders/deletedorderslist', areaId).then(function(response, status) {
            console.log(response.data);
            $scope.allItems = response.data;
            // console.log($scope.selectedDate);
            var selectedDate = $scope.selectedDate;
        ...
        $scope.openStatusModal = function(order) {
          ...
          console.log("deliveryDate", order, order.deliveryDate);
          $scope.selectedOrderItem = order;
          $scope.orderStatusObj.orderId = order.order_id;
          $scope.showOrderStatusModal = true;
        }
...
colorAdminApp.controller('returnGarmentsController', ['$scope', '$http', 'NgTableParams', 'ngNotify', 'Auth', function($scope, $http, NgTableParams, ngNotify, Auth) {
  var storeId = Auth.getAreaId();
  $scope.garments = [];
  $scope.garment = { inBarCode: '', message: '' };
  $scope.applyGlobalSearch = function() {
    $scope.tableParams.filter({ $: $scope.search });
  }
  var areaId = Auth.getAreaId();
  ...
  $scope.getReturnGarments = function() {
    $http.post(ldh + 'store-return-garments', { 'storeId': storeId })
    .success(function(response, status) {
      $scope.garments = response.garments;
      $scope.tableParams = new NgTableParams({ count: 20, sorting: { date: "dsc" } }, { counts: [20, 50, 100], dataset: response.garments });
    }).error(function(response, status) {
      console.log(err);
    });
  };
  $scope.returnGarment = function() {
    $scope.garment.status = 'returned';
       // $scope.garment.message = 'store admin returned ';
       $http.post(ldh + 'store-return-garment', $scope.garment)
       .success(function(response, status) {
    ...
    $scope.getReturnGarments();
    $scope.returnGarmentFlat = false;
    $scope.returnGarmentToggle = function(inBarCodeValue) {
      $scope.returnGarmentFlat = !$scope.returnGarmentFlat;
      $scope.garment.inBarCode = inBarCodeValue;
      $scope.garment.apply;
    }
    $scope.returnGarmentChangeStatus = function() {
      if ($scope.garment.secondStatus == 'SAA') {
        $scope.returnGarment();
        $scope.returnGarmentFlat = false;
      } else {
        ngNotify.set("you rejected the return garment" + $scope.garment.inBarCode);
      }
    }
    $scope.deleteReturnGarment = function(inBarCodeValue) {
      if (confirm("are you want to delete return garment of " + inBarCodeValue)) {
        $scope.garment = { inBarCode: inBarCodeValue };
        $http.post(ldh + 'store-return-garment-delete', $scope.garment).success(function(response, status) {
...
colorAdminApp.controller('HoldGarmentsController', ['$scope', '$http', 'NgTableParams', 'ngNotify', 'Auth', '$state', function($scope, $http, NgTableParams, ngNotify, Auth, $state) {
  var empId = Auth.getEmpId();
  var areaId = Auth.getAreaId();
  $scope.holdGarment = { inBarCode: '', status: '', secondStatus: '', statusList: ['HG-SA', 'HG-SPA', 'HG-D'] };
  $scope.holdGarments = [];
  var holdGarment = $scope.holdGarment;
  $scope.applyGlobalSearch = function() {
    $scope.tableParams.filter({ $: $scope.search });
  }
  $scope.getHoldGarments = function(areaId) {
    $http.post(ldh + 'store-hold-garments', { 'storeId': areaId }).success(function(response, status) {
      console.log(response);
      $scope.holdGarments = response.garments;
      $scope.holdGarments.apply;
    }).error(function(err) {
      console.log(err);
    });
  }
...
colorAdminApp.controller('OrderReturnGarmentsController', ['$scope', '$http', '$stateParams', '$state', function($scope, $http, $stateParams, $state) {
  'use strict';
  console.log('OrderReturnGarmentsController')
  var orderId = $stateParams.orderId;
  $scope.returnGarments = [];
  $scope.getResutnGarments = function($o) {
    $http.post(ldh + 'order-return-garments', { 'orderId': orderId }).success(function(response, status) {
      $scope.returnGarments = response.garments;
    }).error(function(error) {
      console.log(error);
    });
  }
  $scope.getResutnGarments(orderId);
}]);
...
colorAdminApp.controller('OrderDetailsController', ['$rootScope', '$scope', '$stateParams', '$http', '$location', 'ngNotify',
  function($rootScope, $scope, $stateParams, $http, $location, ngNotify) {
    console.log('OrderDetailsController');
    var orderId = $stateParams.orderId || $stateParams.orderId;
...
colorAdminApp.controller('ProcessOrderController', ['$rootScope', '$scope', '$stateParams', '$http', '$location', 'ngNotify',
  function($rootScope, $scope, $stateParams, $http, $location, ngNotify) {
    console.log('ProcessOrderController');
    var orderId = $stateParams.orderId;
    $scope.key = { "code": "290816094956798" };
    ...
    $scope.closeOrderData = function() {
      $scope.itemSelectedType = "";
    ...
    $scope.submitOrderDetails = function() {
      ...
      if (validation) {
        ...
        // $scope.printOrderData = function(printableArea) {
        //  var node = document.getElementById("printableArea");
        //  domtoimage.toPng(node,{width:130})
        //     .then(function (dataUrl) {
        //         var img = new Image();
        //         img.src = dataUrl;
        //         var popupWin = window.open('', '_blank');
        //    popupWin.document.open();
        //         popupWin.document.write('<html><head><link rel="stylesheet" type="text/css" href="assets/css/barcode.css" /></head><body onload="window.print()"><div id="printableArea"> <img class="full-image" src="' + dataUrl + '"> </div> </body></html>');
        ...
        // }
        $scope.showPrintSection = false;
        $scope.togglePrintSection = function(item) {
        ...
        $scope.printOrderData = function(printableArea) {
          ...
          var popupWin = window.open('', '_blank');
          popupWin.document.open();
          popupWin.document.write('<html><head><link rel="stylesheet" type="text/css" href="assets/css/barcode.css" /></head><body onload="window.print()"><div id="printableArea">' + printContents + '</div> </body></html>');
          popupWin.document.close();
        }
            //$scope.printOrderData = function(){
            //window.print() 
            //};
            $scope.generateBarcodesForOrderItems = function() {
              $scope.selectedItems = $scope.processOrderDetails.processItems;
              $scope.showPrintSection = true;
            };
          }
          ]);
colorAdminApp.factory('Auth', function() {
  if (localStorage.getItem('laundry_admin_user_obj') !== null) {
    var cbsLocal = localStorage.getItem('laundry_admin_user_obj');
    var localObj = $.parseJSON(cbsLocal);
        //console.log(localObj);
        var empObj = localObj.employee;
      } else if (localStorage.getItem('cu_user_obj') !== null) {
      ...
      var employeeObj = {};
      employeeObj.getCityId = function() {
        var cityId = 0;
        var cityId = empObj.cityId;
        return cityId;
      },
      employeeObj.getAreaId = function() {
        return empObj.areaId;
      },
      employeeObj.getEmpId = function() {
        return empObj.id;
      },
      employeeObj.getRole = function() {
        return empObj.role;
      }
      return employeeObj;
    });
...
/* -------------------------------
 ...
 ------------------------------- */
 colorAdminApp.controller('headerController', function($scope, $rootScope, $state, $http, Auth) {
  var areaId = $scope.userAreaId;
     // $scope.today = new Date();
     ...
     if ($scope.userObj && $scope.userObj.employee) {
      ...
      $scope.userStore = $scope.userObj.employee.store;
      $scope.userAreaId = $scope.userObj.employee.areaId;
    }
    $scope.cuObj = angular.fromJson(localStorage.getItem('laundry_admin_user_obj'));
    if ($scope.cuObj && $scope.cuObj.employee) {
      $scope.userId = $scope.cuObj.employee.id;
      $scope.userRole = $scope.cuObj.employee.role;
    }
    console.log('checking');
    var emp_id = $scope.userId;
    var checkin = new Date();
    ...
    $scope.check();
    $scope.getSearchResults = function(searchKey, areaId) {
      $http.post(ldh + 'global-search', { searchInput: searchKey, 'areaId': $scope.userAreaId })
      .success(function(response, status) {
        $rootScope.searchResults = response;
        $state.go('app.global-search');
    ...
    $scope.payload = { empId: userId, logoutTime: logouttime};
    $scope.logout = function() {
      $http.post(ldh + 'store-logout', $scope.payload).success(function(response, status) {
        $rootScope.logout = response;
        console.log('logout');
            // localStorage.removeItem('laundry_admin_user_obj');
            // localStorage.removeItem('cu_user_obj');
            // localStorage.removeItem('myResource');
            window.localStorage.clear();
            $state.go('login');
          })      
    }
  });
 colorAdminApp.controller('SearchResultsController', function($scope, $rootScope, $state, $http) {
  ...
  $scope.showModal = false;
  $scope.toggleModal = function(orderId, wallet, balanceAmount) {
    $scope.showModal = !$scope.showModal;
    $scope.payment.oid = orderId;
    $scope.payment.wallet = wallet;
    $scope.payment.balanceAmount = balanceAmount;
  };
  $scope.makePayment = function() {
    $scope.showModal = false;
    $http.post(ldh + 'admin/orders/makepayment', $scope.payment).then(function(response, status) {
...
/* -------------------------------
 ...
 ------------------------------- */
colorAdminApp.controller('DashboardController', ['$scope', '$rootScope', '$state', '$http', 'Auth', function($scope, $rootScope, $state, $http, Auth) {
  $scope.dashboard = {};
  ...
  $scope.sc = { 'storeId': storeId, 'fromDate': '', 'toDate': '', 'toDay': '' };
  var monthsList = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
  $scope.dateSelected = new Date().getDate() + '/' + monthsList[new Date().getMonth()] + '/' + new Date().getFullYear();
  $('#scFromDate').datepicker({
    setDate: new Date(),
    todayHighlight: true,
    format: 'dd/MM/yyyy'
  });
  ...
  $scope.storeCollectionf = function() {
    $scope.sc.fromDate = new Date($('#scFromDate').val());
    $scope.sc.toDate = new Date($('#scToDate').val());
    storeCollection($scope.sc);
  }
  function storeCollection(sc) {
    $http.post(ldh + 'store-collections', sc).success(function(response, state) {
      $scope.sc.storeCollections = response;
    ...
    $scope.doFilter = function() {
      ...
      $scope.orderPayLoad.fromDate = fromDate;
      $scope.orderPayLoad.toDate = toDate;
      getDashBoard($scope.orderPayLoad);
    };
    function getDashBoard(sp) {
      $http.post(ldh + 'store-dashboard', sp).success(function(response, state) {
        $scope.dashboard = response;
        console.log("response table", response);
        if (roleName == 'STORE_ADMIN') {
          $scope.toDayTarget();
          $scope.dashboardType = 'storeDashboard';
          $scope.dcTotal = 0;
        }
      }).error(function(err, status, headers) {
        console.log(err, status, headers);
    ...
    getDashBoard($scope.orderPayLoad);
    $scope.toDayTarget = function() {
      console.log("$scope.dashboard.toDayTarget", $scope.dashboard.toDayTarget)
      $scope.result = $scope.dashboard.toDayTarget;
      $scope.detailsIn.type = 'toDayTarget';
      $scope.result.apply;
    };
    $scope.monthTarget = function() {
      console.log("$scope.dashboard.monthTarget", $scope.dashboard.monthTarget)
      $scope.result = $scope.dashboard.monthTarget;
      $scope.detailsIn.type = 'monthTarget';
      $scope.result.apply;
    };
    $scope.lastMonthTarget = function() {
      console.log("$scope.dashboard.lastMonthTarget", $scope.dashboard.lastMonthTarget)
      $scope.result = $scope.dashboard.lastMonthTarget;
      $scope.detailsIn.type = 'lastMonthTarget';
      $scope.result.apply;
    };
    $scope.showModal = false;
...
colorAdminApp.controller('SuperAdminDashboardController', ['$scope', '$rootScope', '$state', '$http', 'Auth', function($scope, $rootScope, $state, $http, Auth) {
  $scope.dashboard = {};
  $scope.userRole = Auth.getRole();
  console.log($scope.userRole);
  $scope.result = {};
  ...
  var checkout = $('#toDatef').datepicker({
    ...
    $scope.getDashBoard = function() {
      $http.post(ldh + 'super-admin-dashboard', $scope.orderPayLoad).success(function(response, state) {
        $scope.dashboard = response;
        ...
        if (typeof(response.cuponsTable) == 'object' && response.cuponsTable && Object.values(response.cuponsTable).length) {
          ...
          var keys = Object.keys(cupnRes[2]); 
        ...
        console.log("response table", response.cuponsTable);
      }).error(function(err, status, headers) {
        console.log(err, status, headers);
      });
    }
...
colorAdminApp.controller('DashboardxController', function($scope, $rootScope, $state) {
    /* Vector Map
    ------------------------- */
    $('#world-map').vectorMap({
      map: 'world_mill_en',
