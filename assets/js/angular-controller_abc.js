...
//var ldh = 'http://192.168.10.110/laundry/';
...
colorAdminApp.controller('FlatController',['$scope','$http','pagination','ngNotify','NgTableParams',function($scope,$http,$pagination,ngNotify, NgTableParams){
    ...
    ------------------------- */
   /* $('#email-to').tagit({
        availableTags: ["c++", "java", "php", "javdscript", "ruby", "python", "c"]
    }); */
...
colorAdminApp.controller('CatalogItemController',['$scope','$http','ls','pagination','ngNotify','NgTableParams',function($scope,$http,$ls,$pagination,ngNotify, NgTableParams){
	...
	$scope.addCatalogItem =  function(){
		...
		$http.post(ldh+'admin/catalog/storeitems',$scope.catalogitem).success(function(response,status){
			...
			ngNotify.set('Your successfully updated '+$scope.catalogitem.name);
			else
			ngNotify.set('Your successfully added '+$scope.catalogitem.name);
...
colorAdminApp.controller('PlaceOrderController',['$scope','$http','ls','pagination','ngNotify','NgTableParams',function($scope,$http,$ls,$pagination,ngNotify, NgTableParams){
	'use strict';
  ...
  $scope.poitems =  [1];
  $scope.placeorder = { service:[],itemtype:''};
  ...
  $scope.removeChoice = function($i) {
	...
	$scope.getAddons = function(){
		console.log('test1');
		if($scope.placeorder.service === undefined)
			return;
			
			$scope.addons =  $ls.getServiceAddons($scope.placeorder).then(function(data){
	...
	$scope.getServiceItems = function(){
		if($scope.placeorder.itemtype === undefined)
			return;

			$scope.items =  $ls.getServiceItems($scope.placeorder).then(function(data){
				$scope.items = data.data;
				$scope.items.apply;
			});
	}									
