...
colorAdminApp.controller('customerEnquiryController',['$scope','$http','ls','ngNotify', '$rootScope', function($scope,$http,$ls,ngNotify, $rootScope){
	'use strict';
...
$scope.quickCalculation = function(){
	...
	console.log($scope.mainorder.qdAmount);	
	$scope.discountAndQuickDelivery = false;
...
$scope.getPreOrder = function($customerId){
	...
	$http.post(ldh+'admin/orders/getpreorder',$scope.placeorder).success(function(response,status){
		...
		console.log($scope.addresses.servicetax);
		if ($scope.addresses.servicetax == true) {
			$scope.mainorder.totalAmount = $scope.subtotal + $scope.subtotal * (14.5 / 100);
		} else {
			$scope.mainorder.totalAmount = $scope.subtotal;
		}
...
if($stateParams.id){
  ...
  $http.post(ldh+'admin/orders/editMainOrder', $scope.orderId).success(function(response,status){
  	...
  	$scope.mainorder.adminDiscountAmount = response.adminDiscountAmount;
  	$scope.mainorder.orderDate 			= response.orderDate;
  	$scope.mainorder.qd 				= response.qd;
  	 	
  	console.log($scope.mainorder.totalAmount);
...
colorAdminApp.controller('PlaceOrdersController',['$scope','$state','$http','NgTableParams','ngNotify','Auth', function($scope, $state,$http,NgTableParams,ngNotify,Auth){
...
colorAdminApp.controller('returnGarmentsController',['$scope','$http','NgTableParams','ngNotify','Auth', function($scope,$http,NgTableParams,ngNotify,Auth){
	...
	$scope.getReturnGarments = function(){
		...
		.success(function(response,status){
			...
			$scope.tableParams = new NgTableParams({ count: 20,  sorting: { id: "dsc" } }, { counts: [20, 50, 100], dataset: response.garments}); 
