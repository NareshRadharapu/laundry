...
/* -------------------------------
 ...
 if(myHost=='www.cbsatwork.com')
 	var ldh = 'http://www.cbsatwork.com/laundry/';
 	 ...
 	 var ldh = 'http://192.168.0.111/laundry/';
	 // var ldh = 'http://www.cbsatwork.com/laundry/';
 /*****************************************************/
 /****************** Login Controller *****************/
 /*****************************************************/
...
colorAdminApp.controller('PlaceOrderController',['$scope','$rootScope' ,'$http','ls','ngNotify','$stateParams','$state', function($scope,$rootScope,$http,$ls,ngNotify, $stateParams,$state){
	...
	$scope.mainorder 	= { orderId:'', customerId:'', temporders:[], orderDate:'',deliveryDate:'', adminDiscount:0, addressId:'', totalAmount:'', qd:false, qdAmount:0 };
...
$scope.quickCalculation = function(){
	...
	$scope.subtotal = $scope.subtotal + $scope.mainorder.qdAmount;	
	$scope.subtotal.apply;
	console.log($scope.subtotal);
}
...
if($stateParams.id){
  ...
  $http.post(ldh+'admin/orders/editMainOrder', $scope.orderId).success(function(response,status){
  	...
  	$scope.mainorder.qd 				= true; response.qd;
...
$scope.temporders.discountChange = function(st , n){
	$scope.subtotal = st - n;
	$scope.mainorder.totalAmount = Math.round(st - n) + (st - n)*(14.5/100);
};
$scope.saveMainOrder = function(){
	...
	}else if($scope.mainorder.deliveryDate == '' || !$scope.mainorder.deliveryDate){
