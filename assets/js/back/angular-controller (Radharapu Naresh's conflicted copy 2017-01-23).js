...
/* -------------------------------
 ...
 if(myHost=='www.cbsatwork.com')
 	var ldh = 'http://www.cbsatwork.com/laundry/';
 	 ...
 	 var ldh = 'http://192.168.0.102/laundry/';
	 // var ldh = 'http://www.cbsatwork.com/laundry/';
 /*****************************************************/
 /****************** Login Controller *****************/
 /*****************************************************/
 	...
 	$scope.login = function(form,data){
		...
		$http.post(ldh+'store-authentication',data).then(function(response,status){
			...
			if(response.status == 202){
				...
				//$rootScope.myResource = response.resource;
...
colorAdminApp.controller('CustomerPackageController',
	['$scope', '$http', '$stateParams', 'NgTableParams', 'ngNotify', 'Auth', 
...
colorAdminApp.controller('CustomerListController',['$scope','$http','pagination','ngNotify','NgTableParams','Auth', function($scope,$http,$pagination,ngNotify, NgTableParams,Auth){
	...
	$scope.customer = {firstname:'',id:'',area:areaId};
	$scope.areas =[];
...
colorAdminApp.controller('PlaceOrderController',['$scope','$rootScope' ,'$http','ls','ngNotify','$stateParams','$state', function($scope,$rootScope,$http,$ls,ngNotify, $stateParams,$state){
	...
	$scope.mainorder 	= { orderId:'', customerId:'', temporders:[], orderDate:'',deliveryDate:'', adminDiscount:0, addressId:1, totalAmount:'', qd:false, qdAmount:0,sms:false };
...
$scope.quickCalculation = function(){
	...
	$scope.discountAndQuickDelivery = false;
	var qdAmount = Number($scope.mainorder.qdAmount || 0);
	console.log(qdAmount);
	var subtotal = Number($scope.subtotal || 0);
	console.log(subtotal);
	var discount = Number($scope.mainorder.adminDiscountAmount || 0);
	console.log(discount);
	$scope.subtotal1 = subtotal + qdAmount - discount;
...
if($stateParams.id){
  ...
  $http.post(ldh+'admin/orders/editMainOrder', $scope.orderId).success(function(response,status){
  	...
  	$scope.mainorder.qdAmount			= Number(response.qdAmount || 0);
  
  		$scope.subtotal = response.subTotal - $scope.mainorder.qdAmount + 
  							$scope.mainorder.adminDiscountAmount;
		...
		*/
		...
		$http.post(ldh+'admin/orders/saveorder',$scope.mainorder).success(function(response,status){
			...
			if($scope.mainorder.sms){
				$http.post(ldh+'api-sms',{}).success(function(response,status){
...
colorAdminApp.controller('PlaceOrdersController',['$scope','$state','$http','NgTableParams','ngNotify',
	'Auth', '$window', '$interval', '$rootScope', function($scope, $state,$http,NgTableParams,ngNotify,Auth,$window, $interval, $rootScope){
	...
	function filterOrderItems(fromDate,toDate,areaCode){
		...
		angular.forEach($scope.allItems,function(val, key){
			// if (val.totalAmount.indexOf(',') != -1) {
			// 		val.totalAmount = val.totalAmount.replace(/,/g , "");
			// }
			// if (val.balanceAmount.indexOf(',') != -1) {
			// 		val.balanceAmount = val.balanceAmount.replace(/,/g , "");
			// }
			// if (val.paidAmount.indexOf(',') != -1) {
			// 		val.paidAmount = val.paidAmount.replace(/,/g , "");
			// }
			// if (val.adminDiscountAmount.indexOf(',') != -1) {
			// 		val.adminDiscountAmount = val.adminDiscountAmount.replace(/,/g , "");
			// }
			// if (val.reFundAmount.indexOf(',') != -1) {
			// 		val.reFundAmount = val.reFundAmount.replace(/,/g , "");
			// }
			// if (val.closingAmount.indexOf(',') != -1) {
			// 		val.closingAmount = val.closingAmount.replace(/,/g , "");
			// }
...
/* -------------------------------
 ...
 ------------------------------- */
 colorAdminApp.controller('headerController', function($scope, $rootScope, $state, $http) {
 	
 	$scope.getSearchResults=function(searchKey){
 		$http.post(ldh+'global-search', {searchInput : searchKey})
 		.success(function(response,status){
...
colorAdminApp.controller('DashboardController',['$scope', '$rootScope', '$state', '$http','Auth', function($scope, $rootScope, $state, $http, Auth) {
 	...
 	$http.post(ldh+'store-dashboard',{'storeId':storeId}).success(function(response, state){
 	...
 	$scope.lastDay = function(){
 		console.log("$scope.dashboard.LastDayOrders",$scope.dashboard.LastDayOrders)
		$scope.result = $scope.dashboard.LastDayOrders;
 		$scope.result.apply;
 	}
 	
 	$scope.lastMonth = function(){
 		console.log("$scope.dashboard.LastMonthOrders",$scope.dashboard.LastMonthOrders)
		$scope.result = $scope.dashboard.LastMonthOrders;
 		$scope.result.apply;
 	}
 	$scope.allRecords = function(){
 		console.log("$scope.dashboard.records",$scope.dashboard.records)
		$scope.result = $scope.dashboard.records;
 		$scope.result.apply;
 	}
